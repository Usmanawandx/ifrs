...
class ReportController extends Controller
{
        ...
        try {
            ...
            if ($request->hasFile('csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (!empty($value[0])) {
                        ...
                        if (!empty($product)) {
                            ...
                            $is_valid =  false;
                            $error_msg = "Product not found in row no. $row_no";
                            break;
                        }
                    } else {
                        $is_valid =  false;
                    ...
                    // converting excel date to mysql date-time format
                    $dateTimeObject = \DateTime::createFromFormat('U', ($value[2] - 25569) * 86400);
                    $mysqlDateTime = $dateTimeObject->format('Y-m-d H:i:s');
                    
                    
                    $temp['ref_no']             = !empty($value[1]) ? $value[1] : null;
                    // $temp['transaction_date']   = !empty($value[2]) ? date('Y-m-d H:i:s', strtotime($value[2])) : null;
                    $temp['transaction_date']   = !empty($value[2]) ? $mysqlDateTime : null;
                    $temp['stock_in']           = !empty($value[3]) ? $value[3] : null;
                    $temp['stock_out']          = !empty($value[4]) ? $value[4] : null;
                    $temp['current_stock']      = !empty($value[5]) ? $value[5] : null;
                ...
                if (!empty($formated_data)) {
                    DB::table('stock_history')->insert($formated_data);
                }
    ...
    public function getOpeningBalanceStock($product_id, $start){
        ...
        $financial_year_start = \Carbon::parse(session()->get("financial_year.start"))->subDay()->toDateString();
            $opening_balance = DB::table('stock_history')->selectRaw('(ifnull(SUM(stock_in), 0) - ifnull(SUM(stock_out), 0)) AS opening_balance')
            ->where('product_id', $product_id)
            ->when($start != $financial_year_start, function ($query) use ($start) {
                return $query->whereDate('transaction_date', '<', $start);
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand', 'unit', 'tax_id', 'type', 
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state','zero','product_type_id','start_date','end_date']);
            ...
            if (!empty($filters['start_date']) && !empty($filters['end_date'])) {
                $filterdate .= " AND DATE(stock_history.transaction_date) BETWEEN '{$filters['start_date']}' AND '{$filters['end_date']}' ";
            }

            if (!empty($filters['product_type_id'])) {
                $filterquery .= " AND products.product_type = '{$filters['product_type_id']}'";
            }
            if (!empty($filters['category_id'])) {
                $filterquery .= " AND products.category_id = '{$filters['category_id']}'";
            }
            if (!empty($filters['sub_category_id'])) {
                $filterquery .= " AND products.sub_category_id = '{$filters['sub_category_id']}'";
            }
            if (!empty($filters['brand'])) {
                $filterquery .= " AND products.brand_id = '{$filters['brand']}'";
            }
            if (!empty($filters['unit'])) {
                $filterquery .= " AND products.unit_id = '{$filters['unit']}'";
            }
            if (empty($filters['zero'])) {
                // $filterquery .= " AND vl.qty_available != 0";
            }

            $query = 'SELECT products.id, products.name, products.sku, 
                        (SELECT stock_history.stock_in from stock_history WHERE stock_history.ref_no = "opening_stock" AND stock_history.product_id = products.id) as opening,
                        (SELECT ifnull(sum(stock_history.stock_in),0) FROM stock_history WHERE stock_history.product_id = products.id AND stock_history.ref_no != "opening_stock" '. $filterdate .' LIMIT 1) as sum_stock_in,
                        (SELECT ifnull(sum(stock_history.stock_out),0) FROM stock_history WHERE stock_history.product_id = products.id '. $filterdate .' LIMIT 1) as sum_stock_out
                        FROM `products`
            ...
            return view('report.partials.stock_report_table')->with(compact('products', 'start'));
            
        }
        
        
        $product_type       = type::where('is_milling', 0)->pluck('name','id'); 
        $categories         = child_cat::pluck('name','id'); 
        ...
        return view('report.stock_report')
            ...
            'show_manufacturing_data','sub_categories','product_type'));
    }
    ...
    public function valuationReport(Request $request){
        ...
        if(request()->ajax()){
            ...
            $asOnDate = $this->transactionUtil->uf_date(trim($request->input('date'), false));     
            $date = Business::first();
            $currentYear = date('Y'); 
            $selectedMonth = str_pad($date->fy_start_month, 2, '0', STR_PAD_LEFT);
            $financial_year_start = \Carbon::parse(session()->get("financial_year.start"))->subDay()->toDateString();
            ...
            //     ->leftJoin('stock_history as sh', 'products.id', '=', 'sh.product_id')
            //     ->leftJoin('valuation_history as vh', 'products.id', '=', 'vh.product_id')
            //     ->select(
            //         'products.id',
            //         'products.sku',
            //         'products.name',
            //         DB::raw("(SELECT IFNULL(SUM(stock_in), 0) - IFNULL(SUM(stock_out), 0) FROM stock_history WHERE product_id = products.id AND DATE(transaction_date) BETWEEN '$financial_year_start' AND '$asOnDate') as current_stock"),
            //         DB::raw('SUM(vh.quantity) as sum_qty'),
            //         DB::raw('SUM(vh.purchase_amount) as sum_pur_amount')
            //     )
            //     ->whereBetween(DB::raw('DATE(vh.transaction_date)'), [$financial_year_start, $asOnDate])
            //     ->groupBy('products.id')
            //     ->get();
            
            $products = DB::table('products')
                ->leftJoin('stock_history as sh', 'products.id', '=', 'sh.product_id')
                ->leftJoin('valuation_history as vh', function ($join) use ($financial_year_start, $asOnDate) {
                    $join->on('products.id', '=', 'vh.product_id')
                        ->whereBetween(DB::raw('DATE(vh.transaction_date)'), [$financial_year_start, $asOnDate]);
                })
                ->select(
                    'products.id',
                ...
                ->where('products.is_non_inventory', 0)
                ->groupBy('products.id')
                ->get();


            return view('report.valuation_report_partial')->with(compact('products'));
        }
        return view('report.valuation_report');
    }

    public function valuationHistory($product_id){

        $valuation_history = DB::table('valuation_history as vh')
                                ->leftjoin('products','products.id','=','vh.product_id')
                                ->where('vh.product_id',$product_id)
                                ->select('vh.*','products.name')
                                ->orderby('vh.transaction_date','asc')
                                ->orderby('vh.transaction_id','asc')
                                ->get();
        return view('report.valuation_history')->with(compact('valuation_history'));
    }

    public function getValuationByDate(Request $request){
        
        $product_id = $request->input('product_id');
        $avg_date   = $request->input('date');
        ...
        $summary = DB::table('valuation_history as vh')
            ->selectRaw('SUM(vh.quantity) as sum_qty, SUM(vh.purchase_amount) as sum_pur_amount')
            ->where('vh.product_id', $product_id)
            ->whereBetween('vh.transaction_date',[$financial_year_start, $avg_date])
            ->first();

        $curr_avg_rate = ($summary->sum_qty != 0) ? ($summary->sum_pur_amount / $summary->sum_qty) : 0;
        return $curr_avg_rate;
    }
    ...
    public function salesSummaryReport()
    {
        ...
        if (request()->ajax()) {
            ...
            $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->leftJoin('sale_type as f', 'transactions.saleType', '=', 'f.id')
                ->leftJoin('contacts as trnanspo', 'transactions.transporter_name', '=', 'trnanspo.id')
                ->leftJoin('contacts as saleman', 'transactions.sales_man', '=', 'saleman.id')
                ->leftJoin('vehicle AS vl', 'transactions.vehicle_no', '=', 'vl.id')
                ->leftJoin('accounts AS tr_acc', 'transactions.transaction_account', '=', 'tr_acc.id')
                ...
                ->leftJoin(
                    'transactions AS DNN',
                    'transactions.delivery_note_no',
                    '=',
                    'DNN.id'
                )
                ->leftJoin(
                    'transactions AS son',
                    'transactions.sale_order_no',
                    '=',
                    'son.id'
                )
                ->leftJoin(
                    'transactions AS sin',
                    'transactions.sale_invoice_no',
                    '=',
                    'sin.id'
                )
                ->leftJoin(
                ...
                ->where('transactions.business_id', $business_id)
                ->where('transactions.type', 'sale_invoice')
                ...
                ->select(
                    ...
                    'transactions.invoice_no',
                    'transactions.add_charges',
                    'transactions.less_charges',
                    'transactions.invoice_no as invoice_no_text',
                    'contacts.name',
                    'contacts.mobile',
                    'f.name as sales_type',
                    'contacts.contact_id',
                    'contacts.supplier_business_name',
                    'contacts.ntn_cnic_no',
                    'trnanspo.supplier_business_name as trnanspo_name',
                    'vl.vhicle_number as vehicle',
                    'transactions.vehicle_no as vehicle_no',
                    'tr_acc.name as transaction_account',
                    'saleman.supplier_business_name as sales_man',
                    'transactions.status',
            ...
            if (!empty(request()->input('type'))) {
                $sells->where('transactions.saleType', request()->input('type'));
            }
            if (!empty(request()->input('transporter'))) {
                $sells->where('transactions.transporter_name', request()->input('transporter'));
            }
            if (!empty(request()->input('vehicle'))) {
                $sells->where('transactions.vehicle_no', request()->input('vehicle'));
            }
            if (!empty(request()->input('salesman'))) {
                $sells->where('transactions.sales_man', request()->input('salesman'));
            }
            if (!empty(request()->input('transaction_account'))) {
                // $sells->where('transactions.transaction_account', request()->input('transaction_account'));
                $contact_id = Account::where('id', request()->input('transaction_account'))->first()->contact_id;
                $sells->where('transactions.contact_id', $contact_id);
            }
            if (!empty(request()->input('control_account'))) {
                $transaction_account = Account::where('account_type_id', request()->input('control_account'));
                if($transaction_account->count()){
                    $sells->whereIn('transactions.contact_id', $transaction_account->pluck('contact_id'));
                }else{
                    $sells->where('transactions.contact_id', 0);
                }
            }
            ...
            if (!empty(request()->input('products'))) {
                $sells->where('tsl.product_id', request()->input('products'));
            }
            
                
            $data =  $sells->groupBy('transactions.id')->with(['sell_lines','sell_lines.product', 'sell_lines.line_tax'])
            ->orderByRaw('transactions.transaction_date ASC')->get();
            
            if(request()->input('summary')){

                return view('report.sale.summary')->with(compact('data'));

            }else if(request()->input('weight') && !request()->input('furtherTax')){

                return view('report.sale.withoutWeight')->with(compact('data'));

            }else if(!request()->input('weight') && request()->input('furtherTax')){

                return view('report.sale.withoutFurther')->with(compact('data'));

            }else if(request()->input('weight') && request()->input('furtherTax')){

                return view('report.sale.withoutWeightFurther')->with(compact('data'));

            }else{
                return view('report.sale.fullDetail')->with(compact('data'));
            }
            
        }
        
        
        $transporters = Contact::where('type', 'transporter')->pluck('supplier_business_name', 'id');
        $customers    = Contact::customersDropdown($business_id, false);
        $saleType     = saletype::All()->pluck('name', 'id');
        $vehicles     = vehicle::All()->pluck('vhicle_number', 'id');
        $salesman     = Contact::where('type', 'Agent')->pluck('supplier_business_name', 'id');
        $accounts     = Account::All()->where('is_closed', 0)->pluck('name', 'id'); 
        $accountsType = AccountType::All()->pluck('name', 'id');
        $products     = Product::All()->pluck('name','id');
        return view('report.salesSummaryReport')
            ->with(compact('business_locations', 'customers','saleType', 'transporters', 'vehicles', 'salesman','products', 'accounts','accountsType'));
    }

    public function saleProductWise()
    {
        $business_id = request()->session()->get('user.business_id');
        $business_locations = BusinessLocation::forDropdown($business_id, false);
        $is_admin = $this->businessUtil->is_admin(auth()->user());
        ...
        if (request()->ajax()) {
            ...
            $sells = Product::with(['sale_lines', 'sale_lines.saletransaction' => function($query){
                if(request()->start_date && request()->end_date){
                    $query->whereBetween('transaction_date', [request()->start_date, request()->end_date]);
                }
                if(request()->transporter){
                    $query->where('transporter_name', request()->transporter);
                }
                if(request()->vehicle){
                    $query->where('vehicle_no', request()->vehicle);
                }
            }, 'sale_lines.brand', 'sale_lines.line_tax', 'sale_lines.further_taxs','sale_lines.saletransaction.contact']);
            
            if(request()->show_zero == 0){
                $sells->whereHas('sale_lines.saletransaction');
            }

            if(request()->start_date && request()->end_date){
                $sells->whereHas('sale_lines.saletransaction', function ($query) {
                    $query->whereBetween('transaction_date', [request()->start_date, request()->end_date]);
                    if(request()->transporter){
                        $query->where('transporter_name', request()->transporter);
                    }
            ...
            if(request()->products){
                $sells->where('id',request()->products);
            }
            if(request()->product_type){
                $sells->where('product_type',request()->product_type);
            }
            if(request()->product_category){
                $sells->where('category_id',request()->product_category);
            }
            if(request()->product_sub_category){
                $sells->where('sub_category_id',request()->product_sub_category);
            }
             
            $data =  $sells->get();
    
            if(request()->input('summary')){
                return view('report.sale.productSummary')->with(compact('data'));
            }else{
                return view('report.sale.productfullDetail')->with(compact('data'));
            }
        ...
        $product_type      = Type::where('is_milling','0')->get();
        $product_caetgory  = child_cat::All();
        $subcategory       = subcategory::All();
        return view('report.sale.saleProductWise')
            ->with(compact('business_locations', 'transporters', 'vehicles','products','product_type','product_caetgory','subcategory'));
    }
    ...
    public function saleReturnDetail(){
        ...
        if (request()->ajax()) {
            ...
            $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->where('transactions.business_id', $business_id)
                ->where('transactions.type', 'sale_return_invoice')
                ->where('transactions.delete_status',1)
                ->select(
                    'transactions.id',
                    'transactions.ref_no',
            ...
            if(request()->input('summary')){
                ...
                return view('report.sale_return.summary')->with(compact('data'));

            }else if(request()->input('weight') && !request()->input('furtherTax')){

                return view('report.sale_return.withoutWeight')->with(compact('data'));

            }else if(!request()->input('weight') && request()->input('furtherTax')){

                return view('report.sale_return.withoutFurther')->with(compact('data'));

            }else if(request()->input('weight') && request()->input('furtherTax')){

                return view('report.sale_return.withoutWeightFurther')->with(compact('data'));

            }else{
                return view('report.sale_return.fullDetail')->with(compact('data'));
            }
        ...
        $accountsType = AccountType::All()->pluck('name', 'id');
        $products     = Product::All()->pluck('name','id');
        return view('report.sale_return.salesSummaryReport')
            ->with(compact('business_locations', 'customers','saleType', 'transporters', 'vehicles', 'salesman','products', 'accounts','accountsType'));
    }

    public function saleReturnProductWise(){
        ...
        if (request()->ajax()) {
            ...
            $sells = Product::with('sale_lines', 'sale_lines.saleReturntransaction', 'sale_lines.brand', 'sale_lines.line_tax', 'sale_lines.further_taxs','sale_lines.saleReturntransaction.contact');
            // $sells->whereHas('sale_lines.transaction', function ($query) {
            //     $query->where('type', 'sale_invoice');
            // });
            if(request()->show_zero == 0){
            $sells->whereHas('sale_lines.saleReturntransaction');
            }
            ...
            if(request()->input('summary')){
                return view('report.sale_return.productSummary')->with(compact('data'));
            }else{
                return view('report.sale_return.productfullDetail')->with(compact('data'));
            }
        ...
        $product_caetgory  = child_cat::All();
        $subcategory       = subcategory::All();
        return view('report.sale_return.saleProductWise')
            ->with(compact('business_locations', 'customers','saleType', 'transporters', 'vehicles', 'salesman','products', 'accounts','accountsType','product_type','product_caetgory','subcategory'));
    }
    public function purchaseReportDetail()
    {
        $is_admin = $this->businessUtil->is_admin(auth()->user());
        ...
        if (request()->ajax()) {
            $purchase_orders = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                    ...
                    ->groupBy('transactions.id')->orderby('transactions.id', 'asc');
            ...
            if (!empty(request()->transporter)) {
                $purchase_orders->where('transactions.transporter_name', request()->transporter);
            }
            if (!empty(request()->vehicle)) {
                $purchase_orders->where('transactions.vehicle_no', request()->vehicle);
            }
            ...
            $data = $purchase_orders->with(['purchase_lines','purchase_lines.product', 'purchase_lines.line_tax'])->get();

            if(request()->input('summary')){
                return view('report.purchase.purchaseSummary')->with(compact('data'));
            }else{
                return view('report.purchase.purchaseDetail')->with(compact('data'));
            }
        ...
        return view('report.purchase.purchaseReport')->with(compact('business_locations','suppliers', 'shipping_statuses','product_type','purchase_type','transporter','vehicles','transaction_accounts','sales_man'));
    ...
    public function purchaseProductReport()
    {
        ...
        if (request()->ajax()) {
            ...
            $sells = Product::with(['purchase_lines', 'purchase_lines.purchasetransaction' => function($query){
                if(request()->start_date && request()->end_date){
                    $query->whereBetween('transaction_date', [request()->start_date, request()->end_date]);
                }
                if(request()->transporter){
            ...
            }, 'purchase_lines.brand', 'purchase_lines.line_tax', 'purchase_lines.further_taxs','purchase_lines.purchasetransaction.contact']);

            if(request()->show_zero == 0){
                $sells->whereHas('purchase_lines.purchasetransaction');
            }

            if(request()->start_date && request()->end_date){
                $sells->whereHas('purchase_lines.purchasetransaction', function ($query) {
                    $query->whereBetween('transaction_date', [request()->start_date, request()->end_date]);
                    if(request()->transporter){
                        $query->where('transporter_name', request()->transporter);
                    }
            ...
            if(request()->input('summary')){
                return view('report.purchase.productSummary')->with(compact('data'));
            }else{
                return view('report.purchase.productDetail')->with(compact('data'));
            }
        }
        ...
        $product_caetgory  = child_cat::All();
        $subcategory       = subcategory::All();
        return view('report.purchase.productWise')
            ->with(compact('business_locations', 'customers','saleType', 'transporters', 'vehicles', 'salesman','products', 'accounts','accountsType','product_type','product_caetgory','subcategory'));
    }
    ...
    public function purchaseReturnDetail()
    {
        ...
        if (request()->ajax()) {
            ...
            if(request()->input('summary')){
                return view('report.purchase_return.purchaseSummary')->with(compact('data'));
            }else{
                return view('report.purchase_return.purchaseDetail')->with(compact('data'));
            }
        ...
        return view('report.purchase_return.purchaseReport')->with(compact('business_locations','suppliers', 'shipping_statuses','product_type','purchase_type','transporter','vehicles','transaction_accounts','sales_man'));
    
    }

    public function purchaseReturnProductWise()
    {
        $business_id = request()->session()->get('user.business_id');
        $business_locations = BusinessLocation::forDropdown($business_id, false);
        $is_admin = $this->businessUtil->is_admin(auth()->user());
        ...
        if (request()->ajax()) {
            ...
            $sells = Product::with('purchase_lines', 'purchase_lines.purchaseReturntransaction', 'purchase_lines.brand', 'purchase_lines.line_tax', 'purchase_lines.further_taxs','purchase_lines.purchaseReturntransaction.contact');

            if(request()->show_zero == 0){
                $sells->whereHas('purchase_lines.purchaseReturntransaction');
            }
            if(request()->products){
                $sells->where('id',request()->products);
            }
            ...
            if(request()->input('summary')){
                return view('report.purchase_return.productSummary')->with(compact('data'));
            }else{
                return view('report.purchase_return.productDetail')->with(compact('data'));
            }
        }
        ...
        $product_caetgory  = child_cat::All();
        $subcategory       = subcategory::All();
        return view('report.purchase_return.productWise')
            ->with(compact('business_locations', 'customers','saleType', 'transporters', 'vehicles', 'salesman','products', 'accounts','accountsType','product_type','product_caetgory','subcategory'));
    }
    
    public function recipeIngredient(){
        
        $data  = DB::table('mfg_recipes')
                ->join('products as p2', 'p2.id', '=', 'mfg_recipes.product_id')
                ->join('mfg_recipe_ingredients', 'mfg_recipe_ingredients.mfg_recipe_id', '=', 'mfg_recipes.id')
                ->join('products as p', 'p.id', '=', 'mfg_recipe_ingredients.product_id')
                ->select('p2.name as recipe_name', 'p.name as ingredient_name', 'mfg_recipe_ingredients.*','mfg_recipes.*')
                ->get();
    
        return view('report.recipeIngredient.main')->with(compact('data'));
    }
    
    public function ledgerSearch(){
        $transaction_accounts = Account::where('is_closed', 0)->orderby('name', 'asc')->with(['account_type', 'account_type.parent_account'])->get();
        // $transaction_accounts = Account::where('is_closed', 0)->orderby('name', 'asc')->pluck('name', 'id');
        return view('report/ledger/ledgerSearch')->with(compact('transaction_accounts'));
    }
    ...
    public function Aging()
    {
        ...
        ->select('transactions.*','receipt_details.receipts as pay_amount', 'contacts.supplier_business_name')
        ->selectRaw('DATEDIFF(NOW(), receipt_details.transaction_date) as outstanding_days')
        ->where('transactions.type', '=', 'sale_invoice')
        ->join('contacts', 'transactions.contact_id', '=', 'contacts.id')
        ->leftJoin('receipt_details', 'transactions.ref_no', '=', 'receipt_details.ref_no')
        ->get();

         return view('report.aging_report',compact('data'));
    }
    
    public function getProfitLossData($accountIds, $start_date, $end_date)
    {
         $financial_year_start = \Carbon::parse(session()->get("financial_year.start"))->subDay()->toDateString();
        
    return Account::whereIn('account_type_id', $accountIds)
        ...
        ->select(
            ...
            DB::raw("(SELECT IFNULL(SUM(account_transactions.amount), 0) FROM account_transactions WHERE account_transactions.account_id = accounts.id AND account_transactions.type = 'debit' AND account_transactions.deleted_at IS NULL AND DATE(account_transactions.operation_date) Between '$start_date' AND '$end_date') - 
                (SELECT IFNULL(SUM(account_transactions.amount), 0) FROM account_transactions WHERE account_transactions.account_id = accounts.id AND account_transactions.type = 'credit' AND account_transactions.deleted_at IS NULL AND DATE(account_transactions.operation_date) Between '$start_date' AND '$end_date') as balance")  
        )
        ->where('is_closed', 0)
    ...
    public function profitLoss()
    {
        if(request()->ajax()){
            ...
            $financial_year_start = \Carbon::parse(session()->get("financial_year.start"))->subDay()->toDateString();
            $start_date_subDay = \Carbon::parse($start_date)->subDay()->toDateString();
            
            $closing = DB::table('products')
                ->leftJoin('stock_history as sh', 'products.id', '=', 'sh.product_id')
                ->leftJoin('valuation_history as vh', function ($join) use ($financial_year_start, $end_date) {
                    $join->on('products.id', '=', 'vh.product_id')
                        ->whereBetween(DB::raw('DATE(vh.transaction_date)'), [$financial_year_start, $end_date]);
                })
                ->selectRaw("(CASE WHEN SUM(vh.quantity) != 0 THEN SUM(vh.purchase_amount) / SUM(vh.quantity) ELSE 0 END) * (SELECT IFNULL(SUM(stock_in), 0) - IFNULL(SUM(stock_out), 0) FROM stock_history WHERE product_id = products.id AND DATE(transaction_date) BETWEEN ? AND ?) as total_amount", [$financial_year_start, $end_date])
                ->where('products.is_non_inventory', 0)
                ->groupBy('products.id')
                ->get()
                ->sum('total_amount'); 

            $sales = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'sales')->pluck('account_id'), $start_date, $end_date);
            $sales_return_discount = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'salesreturn&discount')->pluck('account_id'), $start_date, $end_date);
            $other_revenue = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'others_revenue')->pluck('account_id'), $start_date, $end_date);
            // $opening_stock = Account::whereIn('account_type_id', DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'goods_sold')->pluck('account_id')->toArray())
            // ->with(['account_transactions' => function ($query) use ($financial_year_start) {
            //     $query->where('operation_date', '=', $financial_year_start);
            // }])
            // ->get()
            // ->sum(function ($account) {
            //     return $account->account_transactions->sum('amount');
            // });
            
            $opening_stock = DB::table('products')
                ->leftJoin('stock_history as sh', 'products.id', '=', 'sh.product_id')
                ->leftJoin('valuation_history as vh', function ($join) use ($financial_year_start, $start_date_subDay) {
                    $join->on('products.id', '=', 'vh.product_id')
                        ->whereBetween(DB::raw('DATE(vh.transaction_date)'), [$financial_year_start, $start_date_subDay]);
                })
                ->selectRaw("(CASE WHEN SUM(vh.quantity) != 0 THEN SUM(vh.purchase_amount) / SUM(vh.quantity) ELSE 0 END) * (SELECT IFNULL(SUM(stock_in), 0) - IFNULL(SUM(stock_out), 0) FROM stock_history WHERE product_id = products.id AND DATE(transaction_date) BETWEEN ? AND ?) as total_amount", [$financial_year_start, $start_date_subDay])
                ->where('products.is_non_inventory', 0)
                ->groupBy('products.id')
                ->get()
                ->sum('total_amount');
            ...
            $manufacturing_expense = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'manufacturing_expense')->pluck('account_id'), $start_date, $end_date);
            $Administrator_expense = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'administration_selling_expense')->pluck('account_id'), $start_date, $end_date);
            $financial_expense = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'financial_expense')->pluck('account_id'), $start_date, $end_date);
            $purchase = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'purchase')->pluck('account_id'), $start_date, $end_date);
            $purchase_retun = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'purchase_return')->pluck('account_id'), $start_date, $end_date);
            $purchase_retun = $this->getProfitLossData(DB::table('default_account')->where('form_type', 'profit&loss')->where('field_type', 'purchase_return')->pluck('account_id'), $start_date, $end_date);
            
            return view('report.finance.profitloss_partial')->with(compact('sales', 'sales_return_discount','other_revenue','opening_stock','manufacturing_expense','Administrator_expense','financial_expense','purchase','purchase_retun','closing'));
        }
        

        return view('report.finance.profitloss');
    }
    
    public function getbalancesheet($accountIds, $start_date, $end_date)
    {
        return  AccountType::whereIn('id', $accountIds)
                ->select(
                    'id',
                    'name',
                    DB::raw("(SELECT IFNULL(SUM(account_transactions.amount), 0) FROM account_transactions WHERE account_transactions.account_id IN (SELECT id FROM accounts WHERE account_type_id = account_types.id) AND account_transactions.type = 'debit' AND account_transactions.deleted_at IS NULL AND DATE(account_transactions.operation_date) Between '$start_date' AND '$end_date') - 
                        (SELECT IFNULL(SUM(account_transactions.amount), 0) FROM account_transactions WHERE account_transactions.account_id IN (SELECT id FROM accounts WHERE account_type_id = account_types.id) AND account_transactions.type = 'credit' AND account_transactions.deleted_at IS NULL AND DATE(account_transactions.operation_date) Between '$start_date' AND '$end_date') as balance")
                )
                ->with(['accounts' => function ($query) use ($start_date, $end_date) {
                    $query->select('id', 'name', 'account_type_id')
                        ->selectRaw("(SELECT IFNULL(SUM(account_transactions.amount), 0) FROM account_transactions WHERE account_transactions.account_id = accounts.id AND account_transactions.type = 'debit' AND account_transactions.deleted_at IS NULL AND DATE(account_transactions.operation_date) Between '$start_date' AND '$end_date') - 
                                    (SELECT IFNULL(SUM(account_transactions.amount), 0) FROM account_transactions WHERE account_transactions.account_id = accounts.id AND account_transactions.type = 'credit' AND account_transactions.deleted_at IS NULL AND DATE(account_transactions.operation_date) Between '$start_date' AND '$end_date') as balance")
                        ->where('is_closed', 0)
                        ->whereNull('deleted_at')
                        ->orderBy('account_number', 'ASC');
    ...
    {
        if(request()->ajax()){
            $showAccounts = request()->detail;
            $showzero = request()->zero;
            $start_date = \Carbon::parse(session()->get("financial_year.start"))->subDay()->toDateString();
            $end_date = \Carbon\Carbon::createFromFormat('d-m-Y',request()->end_date)->format('Y-m-d');
            $fixed_assets = $this->getbalancesheet(DB::table('default_account')->where('form_type', 'balance_sheet')->where('field_type', 'fixed_assets')->pluck('account_id'), $start_date, $end_date);
            $current_assets = $this->getbalancesheet(DB::table('default_account')->where('form_type', 'balance_sheet')->where('field_type', 'current_assets')->pluck('account_id'), $start_date, $end_date);
            $current_liability = $this->getbalancesheet(DB::table('default_account')->where('form_type', 'balance_sheet')->where('field_type', 'current_liability')->pluck('account_id'), $start_date, $end_date);
            $long_term_liability = $this->getbalancesheet(DB::table('default_account')->where('form_type', 'balance_sheet')->where('field_type', 'long_term_liability')->pluck('account_id'), $start_date, $end_date);
            $owners_equity = $this->getbalancesheet(DB::table('default_account')->where('form_type', 'balance_sheet')->where('field_type', 'equity')->pluck('account_id'), $start_date, $end_date);
            
            $opening_stock = DB::table('products')
                ->leftJoin('stock_history as sh', 'products.id', '=', 'sh.product_id')
                ->leftJoin('valuation_history as vh', function ($join) use ($start_date, $end_date) {
                    $join->on('products.id', '=', 'vh.product_id')
                        ->whereBetween(DB::raw('DATE(vh.transaction_date)'), [$start_date, $end_date]);
                })
                ->selectRaw("(CASE WHEN SUM(vh.quantity) != 0 THEN SUM(vh.purchase_amount) / SUM(vh.quantity) ELSE 0 END) * (SELECT IFNULL(SUM(stock_in), 0) - IFNULL(SUM(stock_out), 0) FROM stock_history WHERE product_id = products.id AND DATE(transaction_date) BETWEEN ? AND ?) as total_amount", [$start_date, $end_date])
                ->where('products.is_non_inventory', 0)
                ->groupBy('products.id')
                ->get()
                ->sum('total_amount');
            ...
            return view('report.finance.balance_sheet_partial',compact('opening_stock','fixed_assets','current_assets','current_liability','long_term_liability','owners_equity', 'showAccounts', 'showzero'));
        }
        return view('report.finance.balance_sheet');
    }
