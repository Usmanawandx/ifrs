        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    $product_array['product_description'] = isset($value[18]) ? $value[18] : null;
                    ...
                    if (isset($value[16])) {
                        ...
                        $product_array['product_custom_field1'] = trim($value[16]);
                    } else {
                    ...
                    if (!empty($unit_name)) {
                        ...
                        if (!empty($unit)) {
                            ...
                            // break;
                            $sub_prefix = substr($value[2], 0, 2);
                            $unit_create = Unit::firstOrCreate(
                                ['actual_name' => $value[2],
                                 'business_id' => 1,
                                 'short_name' => $sub_prefix,
                                 'allow_decimal' => 1,
                                 'created_by' => 1
                                ]
                            );
                            $product_array['unit_id'] = (isset($unit_create->id)) ? $unit_create->id : $unit_create;
                    ...
                    $category_id = trim($value[4]);
                    $category_check=category_list::where('name',$category_name)->first();
                    ...
                    if (!empty($category_name)) {
                        $category = category_list::firstOrCreate(
                    ...
                    $product_array['category_id'] = $category_check->id;


                    }

                    //Add Sub-Category
                    $sub_category_name = trim($value[5]);

                    // dd($sub_category_name);
                    $sub_category_id = trim($value[6]);

                    $sub_category_check=subcategory::where('name',$sub_category_name)->first();
                    if($sub_category_check==null)
                    {
                    ...
                    if (!empty($sub_category_name)) {
                        $sub_category = subcategory::firstOrCreate(
                            ...
                            ['id'=> $sub_category_id,'parent_id' => $product_array['category_id'],'name'=> $sub_category_name]
                        );
                    ...
                    $product_array['sub_category_id'] = $sub_category_check->id;

                }
                    
                    
                        //Add child_category
                    $child_name = trim($value[7]);
                    $child_id = trim($value[8]);

                    $child_check=child_cat::where('name',$child_name)->first();
                    if($child_check==null)
                    {
                    ...
                    {
                    if (!empty($sub_category_name)) {
                        $child_category = child_cat::firstOrCreate(
                            ['id'=>$child_id,'name' => $child_name,'sub'=>$product_array['sub_category_id'] ]
                        );
                        $product_array['child_cat'] = $child_category->id;
                    }
                }
                }else{

                    $product_array['child_cat'] = $child_check->id;
                    ...
                    if (isset($value[15])) {
                        ...
                        $product_array['weight'] = trim($value[15]);
                    } else {
                    ...
                    $purchase_types= purchasetype::where('Type',$value[13])->first();
                    if($purchase_types != null)
                    {
                        if (isset($value[13])) 
                        {
                            $product_array['types'] = $purchase_types->id;
                        } 
                    }else {
                        $sub_prefix = substr($value[13], 0, 2);
                        $product_typess = purchasetype::firstOrCreate(
                            ['Type' => $value[13],'prefix'=> $sub_prefix]
                        );
                        $product_array['types'] = (isset($product_typess->id)) ? $product_typess->id : $product_typess;
                    }



                     $product_types=Type::where('name',$value[14])->first();
                    ...
                    if (isset($value[14])) 
                    {
                        ...
                        $product_array['product_type'] = $product_types->id;
                    } 
                    }else {
                        
                        $sub_prefix = substr($value[14], 0, 2);
                        $product_typess = Type::firstOrCreate(
                            ['name' => $value[14],'type'=>'none','prefix'=> $sub_prefix,'purchase_type' =>$product_typess->id]
                        );
                        
                        $product_array['product_type'] = $product_typess;
                    ...
                    if ($product_array['type'] == 'single') {
                        ...
                        // dd($product_array['variation']['profit_percent']);
