...
class OpeningStockController extends Controller
{
        ...
        if (!empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                ...
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['weight'] = $purchase_line->weight;
                }
    ...
    public function addUpdateOpeningStock(Request $request)
    {
        // dd($request);
        // if (!auth()->user()->can('product.opening_stock')) {
        //     abort(403, 'Unauthorized action.');
        ...
        try {
            ...
            $product = Product::where('business_id', $business_id)->where('id', $product_id)->with(['variations', 'product_tax'])->first();
            ...
            if (!empty($product) && $product->enable_stock == 1) {
                ...
                $transaction_date = \Carbon::parse(session()->get("financial_year.start"))->subDay()->toDateString();
                DB::beginTransaction();
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        ...
                        foreach ($value as $k => $rows) {
                            foreach ($rows as $key => $v) {
                                ...
                                $old_quantity          = $this->productUtil->num_uf(trim($v['old_quantity']));
                                $purchase_line = null;
                                ...
                                if (isset($v['purchase_line_id'])) {
                                   ...
                                   //Quantity = remaining + used
                                    ...
                                    if ($qty_remaining != 0) {
                                        ...
                                        $variation = Variation::where('id', $product->variations[0]->id)->where('product_id', $product->id)->first(); 
                                        
                                        $variation_location_d = VariationLocationDetails::where('variation_id', $product->variations[0]->id)
                                        ...
                                        $variation_location_d->qty_available -= ($old_quantity); 
                                        $variation_location_d->save();
                                        
                                        $variation_location_d->qty_available += ($qty_remaining);
                                        $variation_location_d->save();
                        ...
                        if (!empty($purchase_lines)) {
                            ...
                            DB::table('stock_history')->where('product_id', $product->id)->where('ref_no', 'opening_stock')->delete();
                            DB::table('valuation_history')->where('product_id', $product->id)->where('ref_no', 'opening_stock')->delete();
                            //For hit stock History
                            foreach($purchase_lines as $pl){
                                $transaction->ref_no = "opening_stock";
                                $this->productUtil->stock_history($pl, $transaction, 1, null);
                                $this->productUtil->valuation_history($pl, $transaction);
                            }
        ...
        if (request()->ajax()) { return ['success' => 1, 'msg' => 'Success' ]; }
