...
class TransactionSellLine extends Model
{
    ...
    {
        return $this->belongsTo(\App\Transaction::class, 'transaction_id')->where('type', 'sale_invoice');
    }
    public function saleReturntransaction()
    {
        return $this->belongsTo(\App\Transaction::class, 'transaction_id')->where('type', 'sale_return_invoice');
    }
