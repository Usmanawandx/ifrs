...
class Util
{
    ...
    */
    ...
    public function format_only_date($date, $show_time = false, $business_details = null)
    {
        $format = !empty($business_details) ? $business_details->date_format : session('business.date_format');
        if (!empty($show_time)) {
            $time_format = !empty($business_details) ? $business_details->time_format : session('business.time_format');
    ...
    public function generateReferenceNumber_req($type, $ref_count, $business_id = null, $default_prefix = null)
    {
        $prefix = '';
    ...
    */
    ...
    { 
        $user_details = $request->only(['surname', 'first_name', 'last_name', 'email',
                    ...
                    , 'id_proof_number', 'permanent_address', 'current_address', 'bank_details','selected_contacts','user_accounts_id']);
                    
                    
        // For accounts ids store
        $user_details['account_id']    = $request->input('transaction_account');
        if(count($request->input('transaction_account')) > 0){  
            $user_details['account_id'] = implode(', ', $request->input('transaction_account'));   
        }
    ...
    public function stock_history($sel_pur_lines, $transac, $type, $ingredients = null)
    {   
            $result = '';
            $data = [
                'product_id'        => $sel_pur_lines['product_id'],
                'ref_no'            => $transac->ref_no,
                'transaction_date'  => $transac->transaction_date,
                'weight'            => !empty($sel_pur_lines['weight']) ? $sel_pur_lines['weight'] : NULL,
            ];
            
            if($transac->type == "purchase_return"){
                $quantity           = $this->num_uf($sel_pur_lines['quantity_returned']);
            }else{
                $quantity1          = $this->num_uf($sel_pur_lines['quantity']);
                $quantity           = strpos($quantity1, '-') !== false ? str_replace('-', '', $quantity1) : $quantity1;
            }

            // valuation / avg price for product opening 
            if($transac->type == "opening_stock"){
                $data['avg_rate_without_tax'] = $sel_pur_lines['pp_without_discount'];
                $data['purchase_rate'] = $sel_pur_lines['pp_without_discount'];
                $variation = Variation::where('product_id', $data['product_id'])->first();
                $variation->dpp_inc_tax = $sel_pur_lines['pp_without_discount'];
                $variation->save();
            }
            
            if ($type == 1) {
                $data['stock_in'] = $quantity;
            } else if ($type == 2) {
                $data['stock_out'] = $quantity;
            }
            $result = DB::table('stock_history')->insert($data);
            
            // code for ingredients 
            if ($ingredients != null && count($ingredients) > 0) {
                foreach ($ingredients as $value) {
                    $parent_id = Variation::where('id', $value['parent_id'])->first();
                    
                    if(!empty($parent_id->product_id) && $parent_id->product_id == $sel_pur_lines['product_id']){

                        $result2 = '';
                        $variation    = Variation::where('id', $value['variation_id'])->get();
                        if (count($variation) > 0) {
                            ...
                            DB::table('stock_history')->where('ref_no', $transac->ref_no)->where('product_id', $variation[0]->product_id)->delete();
                
                            $transaction2    = Transaction::where('opening_stock_product_id', $variation[0]->product_id)->get();
                            if ($transaction2->count() > 0) {
                                $purchase_line2  = PurchaseLine::where('transaction_id', $transaction2[0]->id)->get();
                                $opening_stock2          = $purchase_line2[0]->quantity;
                                
                                if($transac->type == "stock_adjustment" && $this->num_uf($value['quantity']) < 0 ){
                                    
                                    $data2['stock_in']      = strpos($this->num_uf($value['quantity']), '-') !== false ? str_replace('-', '', $this->num_uf($value['quantity'])) : $this->num_uf($value['quantity']);
                                }else{
                                    $data2['stock_out']      = $this->num_uf($value['quantity']);
                                }
                                $data2['current_stock']  = ($opening_stock2 - $this->num_uf($value['quantity']));
                                $result2 = DB::table('stock_history')->insert($data2);
                            }else{
                                $opening_stock2          = 0;
    ...
    public function valuation($sel_pur_lines, $transac)
    {
        ...
        if($transac->type == 'Purchase_invoice'){

            $purchase_invoice = DB::table('purchase_lines')
                                    ->join('transactions', 'transactions.id', '=', 'purchase_lines.transaction_id')
                                    ->selectRaw('SUM(quantity) as total_quantity')
                                    ->selectRaw('SUM(pp_without_discount) as unitpricetotal')
                                    ->selectRaw('(SUM(pp_without_discount * quantity)) as totalvalue')
                                    ->where('product_id', $data['product_id'])
                                    ->where('transaction_id', '>', 0)
                                    ->where(function ($query){
                                        $query->where('transactions.type', 'Purchase_invoice')
                                                ->orWhere('transactions.type', 'opening_stock');
                                    })
                                    ->first();
            
            $old_purchase_amount      =   $purchase_invoice->totalvalue;
            
            $current_purchase_amount  =  ($sel_pur_lines['quantity'] * $sel_pur_lines['pp_without_discount']);
            
            $new_purchase_quantity    =  ($purchase_invoice->total_quantity + $sel_pur_lines['quantity']);
            
            $new_purchase_amount      =  ($old_purchase_amount + $current_purchase_amount);
            
            $avg_rate                 =  ($new_purchase_amount / $new_purchase_quantity);
            
            $data['avg_rate_without_tax'] = $avg_rate;
                
            $variation = Variation::where('product_id', $data['product_id'])->first();
            $variation->dpp_inc_tax = $avg_rate;
            $variation->save();
        ...
        if($transac->type == "purchase_return"){
            ...
            $purchase_invoice = DB::table('purchase_lines')
                                    ->join('transactions', 'transactions.id', '=', 'purchase_lines.transaction_id')
                                    ->selectRaw('(SUM(quantity) - SUM(quantity_returned)) as total_quantity')
                                    ->selectRaw('(SUM(pp_without_discount * (quantity-quantity_returned))) as totalvalue')
                                    ->where('product_id', $data['product_id'])
                                    ->where('transaction_id', '>', 0)
                                    ->where(function ($query) {
                                        $query->where('transactions.type', 'Purchase_invoice')
                                    ...
                                    ->first();
            if($purchase_invoice->totalvalue > 0 && $purchase_invoice->total_quantity > 0){
                $avg_rate                 =  ($purchase_invoice->totalvalue / $purchase_invoice->total_quantity);
            }else{
                $avg_rate = 0;
            }
            $data['avg_rate_without_tax'] = $avg_rate;
        ...
        if($transac->type == "production_purchase"){
            //Production Product ID
            $data['prd_pro_id'] = $sel_pur_lines['variation_id'];

            $production_purchase = DB::table('purchase_lines')
                                    ->join('transactions', 'transactions.id', '=', 'purchase_lines.transaction_id')
                                    ->selectRaw('SUM(quantity) as total_quantity')
                                    ->selectRaw('SUM(pp_without_discount) as unitpricetotal')
                                    ->selectRaw('(SUM(pp_without_discount * quantity)) as totalvalue')
            ...
            $old_purchase_amount      =   $production_purchase->totalvalue;
            
            $current_purchase_amount  =  ($sel_pur_lines['quantity'] * $sel_pur_lines['production_total']);
            
            $new_purchase_quantity    =  ($production_purchase->total_quantity + $sel_pur_lines['quantity']);
        ...
        $grn_ref_no = Transaction::where('id', $transac->purchase_order_ids)->first()->ref_no ?? 0;
        if($grn_ref_no){
            DB::table('stock_history')->where(['ref_no' => $grn_ref_no, 'product_id' => $data['product_id']])
                ->update(['avg_rate_without_tax' => $avg_rate, 'purchase_rate' => $sel_pur_lines['pp_without_discount']]);
        }
        return true;
    }
    ...
    public function valuation_history($sel_pur_lines, $transac){
        $checkNonInventory = Product::where(['id' => $sel_pur_lines['product_id'], 'is_non_inventory' => 1])->count();
        if($checkNonInventory){
           return false; 
        }
        ...
        $data = [
            ...
            'transaction_type'   => $transac->type,
            'rate'               => $this->num_uf($sel_pur_lines['pp_without_discount'])
        ];

        if($transac->type == "purchase_return"){
            $quantity            = '-'.$this->num_uf($sel_pur_lines['quantity_returned']);
        }else{
            $quantity            = $this->num_uf($sel_pur_lines['quantity']);
        }

        $data['quantity']        = $quantity;
        $data['purchase_amount'] = ($data['rate'] * $data['quantity']);

        $result = DB::table('valuation_history')->insert($data);
    ...
    public function sale_invoice_ledger_entries($input, $transaction, $final_total, $user_id){
        
        $old_sell_entgry = AccountTransaction::where("transaction_id",$transaction->id);
        if($old_sell_entgry->count() > 0){
            AccountTransaction::where("transaction_id",$transaction->id)->forceDelete();
        }
        
        $sale_invoice__sales_account                = DB::table('default_account')->where('form_type','sale_invoice')->where('field_type','sales_account')->first()->account_id ?? 0;
        $sale_invoice__sales_tax_input_and_output   = DB::table('default_account')->where('form_type','sale_invoice')->where('field_type','sales_tax')->first()->account_id ?? 0;
        $sale_invoice__sale_tax_further_tax         = DB::table('default_account')->where('form_type','sale_invoice')->where('field_type','further_tax')->first()->account_id ?? 0;
        $sale_invoice__sales_man_commission_exp     = DB::table('default_account')->where('form_type','sale_invoice')->where('field_type','salesman_commission')->first()->account_id ?? 0;
        $sale_invoice__vehicle_exp_account          = DB::table('default_account')->where('form_type','sale_invoice')->where('field_type','vehicle_exp')->first()->account_id ?? 0;
        $sale_invoice__contractor_exp               = DB::table('default_account')->where('form_type','sale_invoice')->where('field_type','contractor_exp')->first()->account_id ?? 0;
        $sale_invoice__sales_discount               = DB::table('default_account')->where('form_type','sale_invoice')->where('field_type','sales_discount')->first()->account_id ?? 0;

        $saleman_comm_amount        = !empty($input['total_salesman_commission']) ? $input['total_salesman_commission'] : 0;
        $sale_acc_amount            = $final_total - $input['total_sale_tax'] - $input['total_further_tax'] + $input['discount_amount'];
        $transaction_date           = $input['transaction_date'];
        $customer_account_ammount   = $final_total;
        $sale_acc_amount            = (!empty($input['add_charges'])) ? $sale_acc_amount - $input['add_charges'] : $sale_acc_amount;
        $sale_acc_amount            = (!empty($input['less_charges'])) ? $sale_acc_amount + $input['less_charges'] : $sale_acc_amount;
        
        // 1st entry
        $get_account_id = Account::where("contact_id",$input['contact_id'])->firstOrFail();
        $sell_no = "sell-".$transaction->id;
        
        $ob_transaction_data = [
            'amount' => $customer_account_ammount,
            'account_id' => $get_account_id->id, //customer_account
            'type' => 'debit',
            'reff_no' => $sell_no,
            'note' => "Sell Generated",
            'against_id' => $sale_invoice__sales_account,
            'sub_type' => 'sell_voucher',
            'po_ref_no' => "N/A",
        ...
        if (isset($input['discount_amount']) && !empty($input['discount_amount']) && $input['discount_amount'] > 0) {
            $ob_transaction_data = [
                'amount' => $input['discount_amount'],
                'account_id' => $sale_invoice__sales_discount, //Discount account
                'type' => 'debit',
                'reff_no' => $sell_no,
                'note' => "Discount",
                'against_id' => $get_account_id->id,
                'sub_type' => 'purchase_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
                'transaction_id' => $transaction->id
        ...
        if(isset($input['add_charges']) && !empty($input['add_charges'])){
            $ob_transaction_data = [
                'amount' => $input['add_charges'],
                'account_id' => $input['add_charges_acc_id'], //add_charges_account
                'type' => 'credit',
                'reff_no' => $sell_no,
                'note' => "Add charges",
                'against_id' => $get_account_id->id,
        ...
        if(isset($input['less_charges']) && !empty($input['less_charges'])){
            $ob_transaction_data = [
                'amount' => $input['less_charges'],
                'account_id' => $input['less_charges_acc_id'], //less_charges_account
                'type' => 'debit',
                'reff_no' => $sell_no,
                'note' => "Less Charges",
                'against_id' => $get_account_id->id,
        ...
        $ob_transaction_data = [
            'amount' => $sale_acc_amount,
            'account_id' => $sale_invoice__sales_account, //sales account
            'type' => 'credit',
            'reff_no' => $sell_no,
            'note' => "Sell Account",
            'against_id' => $get_account_id->id,
        ...
        if(isset($input['total_sale_tax']) && !empty($input['total_sale_tax'])){
            $ob_transaction_data = [
                'amount' => $input['total_sale_tax'],
                'account_id' => $sale_invoice__sales_tax_input_and_output, // sales tax input and output
                'type' => 'credit',
                'reff_no' => $sell_no,
                'note' => "Sales tax",
                'against_id' => $get_account_id->id,
        ...
        if(isset($input['total_further_tax']) && !empty($input['total_further_tax'])){
            $ob_transaction_data = [
                'amount' => $input['total_further_tax'],
                'account_id' => $sale_invoice__sale_tax_further_tax, // sale tax further tax
                'type' => 'credit',
                'reff_no' => $sell_no,
                'note' => "Further tax",
                'against_id' => $get_account_id->id,
        ...
        if(isset($input['sales_man']) && !empty($input['sales_man']) && !empty($saleman_comm_amount)){
            // 4st entry
            $get_saleMan_account_id = Account::where("contact_id",$input['sales_man'])->firstOrFail();
            $ob_transaction_data = [
                'amount' => $saleman_comm_amount,
                'account_id' => $sale_invoice__sales_man_commission_exp, //Sales man Commission EXP ID
                'type' => 'debit',
                'reff_no' => $sell_no,
                'note' => "Commission EXP",
                'against_id' => $get_saleMan_account_id->id,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
            ...
            $ob_transaction_data = [
                'amount' => $saleman_comm_amount,
                'account_id' => $get_saleMan_account_id->id, //salesman account payble
                'type' => 'credit',
                'reff_no' => $sell_no,
                'note' => "Commission EXP",
                'against_id' => $sale_invoice__sales_man_commission_exp,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
        ...
        if(isset($input['transporter_name']) && !empty($input['transporter_name']) && !empty($input['total_transporter_rate'])){
            // 5st entry
            $get_account_transporter = Account::where("contact_id",$input['transporter_name'])->firstOrFail();
            $ob_transaction_data = [
                'amount' => $input['total_transporter_rate'],
                'account_id' => $sale_invoice__vehicle_exp_account, //vehicle Exp Account ID
                'type' => 'debit',
                'reff_no' => $sell_no,
                'note' => "Vehicle Exp",
                'against_id' => $get_account_transporter->id,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
            ...
            $ob_transaction_data = [
                'amount' => $input['total_transporter_rate'],
                'account_id' => $get_account_transporter->id, //vehicle payble account
                'type' => 'credit',
                'reff_no' => $sell_no,
                'note' => "Vehicle Exp",
                'against_id' => $sale_invoice__vehicle_exp_account,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
        ...
        if(isset($input['contractor']) && !empty($input['contractor']) && !empty($input['total_contractor_rate'])){
            // 6st entry
            $get_account_id_contractor = Account::where("contact_id",$input['contractor'])->firstOrFail();
            $ob_transaction_data = [
                'amount' => $input['total_contractor_rate'],
                'account_id' => $sale_invoice__contractor_exp,//Contractor EXP ID
                'type' => 'debit',
                'reff_no' => $sell_no,
                'note' => "Contractor EXP",
                'against_id' => $get_account_id_contractor->id,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
            ...
            $ob_transaction_data = [
                'amount' => $input['total_contractor_rate'],
                'account_id' => $get_account_id_contractor->id, //contractor payble account
                'type' => 'credit',
                'reff_no' => $sell_no,
                'note' => "Contractor EXP",
                'against_id' => $sale_invoice__contractor_exp,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
        ...
        if(isset($input['prd_contractor']) && !empty($input['prd_contractor']) && !empty($input['total_prd_contractor_rate'])){
            // 6st entry
            $get_account_id_contractor = Account::where("contact_id",$input['prd_contractor'])->firstOrFail();
            $ob_transaction_data = [
                'amount' => $input['total_prd_contractor_rate'],
                'account_id' => $sale_invoice__contractor_exp,//Contractor EXP ID
                'type' => 'debit',
                'reff_no' => $sell_no,
                'note' => "Production Contractor EXP",
                'against_id' => $get_account_id_contractor->id,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
    ...
    public function sale_return_ledger_entries($input, $transaction, $final_total, $user_id){
        ...
        $sale_return__sales_account                = DB::table('default_account')->where('form_type','sale_return_invoice')->where('field_type','sales_account')->first()->account_id ?? 0;
        $sale_return__sales_tax_input_and_output   = DB::table('default_account')->where('form_type','sale_return_invoice')->where('field_type','sales_tax')->first()->account_id ?? 0;
        $sale_return__sale_tax_further_tax         = DB::table('default_account')->where('form_type','sale_return_invoice')->where('field_type','further_tax')->first()->account_id ?? 0;
        $sale_return__sales_man_commission_exp     = DB::table('default_account')->where('form_type','sale_return_invoice')->where('field_type','salesman_commission')->first()->account_id ?? 0;
        $sale_return__vehicle_exp_account          = DB::table('default_account')->where('form_type','sale_return_invoice')->where('field_type','vehicle_exp')->first()->account_id ?? 0;
        $sale_return__contractor_exp               = DB::table('default_account')->where('form_type','sale_return_invoice')->where('field_type','contractor_exp')->first()->account_id ?? 0;
        $sale_return__sales_discount               = DB::table('default_account')->where('form_type','sale_return_invoice')->where('field_type','sales_discount')->first()->account_id ?? 0;

        $saleman_comm_amount        = !empty($input['total_salesman_commission']) ? $input['total_salesman_commission'] : 0;
        $sale_acc_amount            = $final_total - $input['total_sale_tax'] - $input['total_further_tax'];
        $transaction_date           = $input['transaction_date'];
        ...
        $get_account_id = Account::where("contact_id",$input['contact_id'])->firstOrFail();
            $sell_no = "sell-return-".$transaction->id; 
            $ob_transaction_data = [
                'amount' => $sale_acc_amount,
                'account_id' => $sale_return__sales_account, //sales account 
                'type' => 'debit',
                'reff_no' => $sell_no,
                'note' => "sales account",
                'against_id' => $get_account_id->id,
            ...
            if(isset($input['total_sale_tax']) && !empty($input['total_sale_tax'])){
                $ob_transaction_data = [
                    'amount' => $input['total_sale_tax'],
                    'account_id' => $sale_return__sales_tax_input_and_output, // sales tax input & output
                    'type' => 'debit',
                    'reff_no' => $sell_no,
                    'note' => "Sales Tax",
                    'against_id' => $get_account_id->id,
            ...
            if(isset($input['total_further_tax']) && !empty($input['total_further_tax'])){
                $ob_transaction_data = [
                    'amount' => $input['total_further_tax'],
                    'account_id' => $sale_return__sale_tax_further_tax, // sales tax further tax
                    'type' => 'debit',
                    'reff_no' => $sell_no,
                    'note' => "Further Tax",
                    'against_id' => $get_account_id->id,
            ...
            if (isset($input['discount_amount']) && !empty($input['discount_amount']) && $input['discount_amount'] > 0) {
                $ob_transaction_data = [
                    'amount' => $input['discount_amount'],
                    'account_id' => $sale_return__sales_discount, //Discount account
                    'type' => 'credit',
                    'reff_no' => $sell_no,
                    'note' => "Discount",
                    'against_id' => $get_account_id->id,
            ...
            $ob_transaction_data = [
                ...
                'type' => 'credit',
                'reff_no' => $sell_no,
                'note' => "sale-return Generated",
                'against_id' => $sale_return__sales_account,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => $user_id,
            ...
            if(isset($input['sales_man']) && !empty($input['sales_man']) && !empty($saleman_comm_amount)){
                ...
                $ob_transaction_data = [
                    ...
                    'reff_no' => $sell_no,
                    'note' => "Commission EXP",
                    'against_id' => $sale_return__sales_man_commission_exp,
                    'sub_type' => 'sell_voucher',
                    'po_ref_no' => "N/A",
                    'operation_date' => $transaction_date,
                    'created_by' => $user_id,
                ...
                $ob_transaction_data = [
                    'amount' => $saleman_comm_amount,
                    'account_id' => $sale_return__sales_man_commission_exp,//Sales man Commission EXP ID
                    'type' => 'credit',
                    'reff_no' => $sell_no,
                    'note' => "Commission EXP",
                    'against_id' => $get_saleMan_account_id->id,
            ...
            if(isset($input['transporter_name']) && !empty($input['transporter_name']) && !empty($input['total_transporter_rate'])){
                ...
                $ob_transaction_data = [
                    ...
                    'reff_no' => $sell_no,
                    'note' => "Sell Generated",
                    'against_id' => $sale_return__vehicle_exp_account,
                    'sub_type' => 'sell_voucher',
                    'po_ref_no' => "N/A",
                    'operation_date' => $transaction_date,
                    'created_by' => $user_id,
                ...
                $ob_transaction_data = [
                    'amount' => $input['total_transporter_rate'],
                    'account_id' => $sale_return__vehicle_exp_account, //vehicle Exp Account ID
                    'type' => 'credit',
                    'reff_no' => $sell_no,
                    'note' => "Sell Generated",
                    'against_id' => $get_account_transporter->id,
            ...
            if(isset($input['contractor']) && !empty($input['contractor']) && !empty($input['total_contractor_rate'])){
                ...
                $ob_transaction_data = [
                    ...
                    'reff_no' => $sell_no,
                    'note' => "Contractor Exp",
                    'against_id' => $sale_return__contractor_exp,
                    'sub_type' => 'sell_voucher',
                    'po_ref_no' => "N/A",
                    'operation_date' => $transaction_date,
                    'created_by' => $user_id,
                ...
                $ob_transaction_data = [
                    'amount' => $input['total_contractor_rate'],
                    'account_id' => $sale_return__contractor_exp,//Contractor EXP ID
                    'type' => 'credit',
                    'reff_no' => $sell_no,
                    'note' => "Contractor Exp",
                    'against_id' => $get_account_id_contractor->id,
    ...
    public function purchase_invoice_ledger_entries($transaction){

         $purchase_account_id          = DB::table('default_account')->where('form_type','Purchase_invoice')->where('field_type','sales_account')->first()->account_id;
         $sales_tax_input_and_output   = DB::table('default_account')->where('form_type','Purchase_invoice')->where('field_type','sales_tax')->first()->account_id;
         $sale_tax_further_tax         = DB::table('default_account')->where('form_type','Purchase_invoice')->where('field_type','further_tax')->first()->account_id;
         $sales_man_commission_exp     = DB::table('default_account')->where('form_type','Purchase_invoice')->where('field_type','salesman_commission')->first()->account_id;
         $vehicle_exp_account          = DB::table('default_account')->where('form_type','Purchase_invoice')->where('field_type','vehicle_exp')->first()->account_id;
         $contractor_exp               = DB::table('default_account')->where('form_type','Purchase_invoice')->where('field_type','contractor_exp')->first()->account_id;
         $discount_id               = DB::table('default_account')->where('form_type','Purchase_invoice')->where('field_type','sales_discount')->first()->account_id;

         $saleman_comm_amount        = !empty($transaction->total_salesman_commission) ? $transaction->total_salesman_commission : 0;
         $purchase_acc_amount        = $transaction->final_total - $transaction->total_sale_tax - $transaction->total_further_tax + $transaction->discount_amount;   
         $purchase_acc_amount        = (!empty($transaction->add_charges)) ? $purchase_acc_amount - $transaction->add_charges : $purchase_acc_amount;
         $purchase_acc_amount        = (!empty($transaction->less_charges)) ? $purchase_acc_amount + $transaction->less_charges : $purchase_acc_amount;
     
         $transaction_date           = $transaction->transaction_date;
         $supplier_account_ammount   = $transaction->final_total;
         ...
         //1st entry
         $get_account_id = Account::where("contact_id", $transaction->contact_id)->firstOrFail();
         $pur_no = "purchase-" . $transaction->id;
         $ob_transaction_data = [
             'amount' => $purchase_acc_amount,
             'account_id' => $purchase_account_id, //purchase account
             'type' => 'debit',
             'reff_no' => $pur_no,
             'note' => "Purchase Invoice Generated",
             'against_id' => $get_account_id->id,
             'sub_type' => 'purchase_voucher',
             'po_ref_no' => "N/A",
             'operation_date' => $transaction_date,
         ...
         if (isset($transaction->total_sale_tax) && !empty($transaction->total_sale_tax)) {
             $ob_transaction_data = [
                 'amount' => $transaction->total_sale_tax,
                 'account_id' => $sales_tax_input_and_output, //Sales tax input & output
                 'type' => 'debit',
                 'reff_no' => $pur_no,
                 'note' => "Sales Tax",
                 'against_id' => $get_account_id->id,
         ...
         if (isset($transaction->total_further_tax) && !empty($transaction->total_further_tax)) {
             $ob_transaction_data = [
                 'amount' => $transaction->total_further_tax,
                 'account_id' => $sale_tax_further_tax, //Sales tax further tax
                 'type' => 'debit',
                 'reff_no' => $pur_no,
                 'note' => "Further tax",
                 'against_id' => $get_account_id->id,
         ...
         if (isset($transaction->add_charges) && !empty($transaction->add_charges)) {
             $ob_transaction_data = [
                 'amount' => $transaction->add_charges,
                 'account_id' => $transaction->add_charges_acc_id, //add_charges_account
                 'type' => 'debit',
                 'reff_no' => $pur_no,
                 'note' => "Add Charges",
                 'against_id' => $get_account_id->id,
         ...
         if (isset($transaction->less_charges) && !empty($transaction->less_charges)) {
             $ob_transaction_data = [
                 'amount' => $transaction->less_charges,
                 'account_id' => $transaction->less_charges_acc_id, //less_charges_account
                 'type' => 'credit',
                 'reff_no' => $pur_no,
                 'note' => "Less Charges",
                 'against_id' => $get_account_id->id,
         ...
         if (isset($transaction->discount_amount) && !empty($transaction->discount_amount)) {
             $ob_transaction_data = [
                 'amount' => $transaction->discount_amount,
                 'account_id' => $discount_id, //Discount account
                 'type' => 'credit',
                 'reff_no' => $pur_no,
                 'note' => "Discount",
                 'against_id' => $get_account_id->id,
         ...
         $ob_transaction_data = [
             'amount' => $supplier_account_ammount,
             'account_id' => $get_account_id->id, // Supplier Account
             'type' => 'credit',
             'reff_no' => $pur_no,
             'note' => "Supplier Account",
             'against_id' => $purchase_account_id,
             'sub_type' => 'sell_voucher',
             'po_ref_no' => "N/A",
             'operation_date' => $transaction_date,
             'created_by' => auth()->user()->id,
         ...
         if (isset($transaction->sales_man) && !empty($transaction->sales_man) && !empty($saleman_comm_amount)) {
             //4st entry
             $get_account_id_saleMan = Account::where("contact_id", $transaction->sales_man)->firstOrFail();
             $ob_transaction_data = [
                 'amount' => $saleman_comm_amount,
                 'account_id' => $sales_man_commission_exp, //Sales man Commission EXP
                 'type' => 'debit',
                 'reff_no' => $pur_no,
                 'note' => "Commission EXP",
                 'against_id' => $get_account_id_saleMan->id,
                 'sub_type' => 'purchase_voucher',
                 'po_ref_no' => "N/A",
                 'operation_date' => $transaction_date,
                 'created_by' => auth()->user()->id,
             ...
             $ob_transaction_data = [
                 'amount' => $saleman_comm_amount,
                 'account_id' => $get_account_id_saleMan->id, //Sales man Account Payable
                 'type' => 'credit',
                 'reff_no' => $pur_no,
                 'note' => "Commission EXP",
                 'against_id' => $sales_man_commission_exp->id,
                 'sub_type' => 'sell_voucher',
                 'po_ref_no' => "N/A",
                 'operation_date' => $transaction_date,
                 'created_by' => auth()->user()->id,
         ...
         if (isset($transaction->transporter_name) && !empty($transaction->transporter_name) && !empty($transaction->total_transporter_rate)) {
             //5st entry
             $get_account_id_transporter = Account::where("contact_id", $transaction->transporter_name)->firstOrFail();
             $ob_transaction_data = [
                 'amount' => $transaction->total_transporter_rate,
                 'account_id' => $vehicle_exp_account, //vehicle Exp Account
                 'type' => 'debit',
                 'reff_no' => $pur_no,
                 'note' => "vehicle Exp",
                 'against_id' => $get_account_id_transporter->id,
                 'sub_type' => 'purchase_voucher',
                 'po_ref_no' => "N/A",
                 'operation_date' => $transaction_date,
                 'created_by' => auth()->user()->id,
             ...
             $ob_transaction_data = [
                 'amount' => $transaction->total_transporter_rate,
                 'account_id' => $get_account_id_transporter->id, //vehicle payable account
                 'type' => 'credit',
                 'reff_no' => $pur_no,
                 'note' => "vehicle Exp",
                 'against_id' => $vehicle_exp_account,
                 'sub_type' => 'sell_voucher',
                 'po_ref_no' => "N/A",
                 'operation_date' => $transaction_date,
                 'created_by' => auth()->user()->id,
         ...
         if (isset($transaction->contractor) && !empty($transaction->contractor) && !empty($transaction->total_contractor_rate)) {
             //6st entry
             $get_account_id_contractor = Account::where("contact_id", $transaction->contractor)->firstOrFail();
             $ob_transaction_data = [
                 'amount' => $transaction->total_contractor_rate,
                 'account_id' => $contractor_exp, //Contractor EXP
                 'type' => 'debit',
                 'reff_no' => $pur_no,
                 'note' => "Contractor EXP",
                 'against_id' => $get_account_id_contractor->id,
             ...
             $ob_transaction_data = [
                 'amount' => $transaction->total_contractor_rate,
                 'account_id' => $get_account_id_contractor->id, //Contractor Payable Account
                 'type' => 'credit',
                 'reff_no' => $pur_no,
                 'note' => "Contractor EXP",
                 'against_id' => $contractor_exp,
                 'sub_type' => 'sell_voucher',
                 'po_ref_no' => "N/A",
                 'operation_date' => $transaction_date,
                 'created_by' => auth()->user()->id,
         ...
         if (isset($transaction->prd_contractor) && !empty($transaction->prd_contractor) && !empty($transaction->total_prd_contractor_rate)) {
            //6st entry
            $get_account_id_contractor = Account::where("contact_id", $transaction->prd_contractor)->firstOrFail();
            $ob_transaction_data = [
                'amount' => $transaction->total_prd_contractor_rate,
                'account_id' => $contractor_exp, //Contractor EXP
                'type' => 'debit',
                'reff_no' => $pur_no,
                'note' => "Production Contractor EXP",
    ...
    public function purchase_return_ledger_entries($request, $purchase_return, $input_data){

        $total_sale_tax             = !empty($request->input('total_sale_tax')) ? $request->input('total_sale_tax') : 0;
        $total_further_tax          = !empty($request->input('total_further_tax')) ? $request->input('total_further_tax') : 0;
        $total_salesman_commission  = !empty($request->input('total_salesman_commission')) ? $request->input('total_salesman_commission') : 0;
        $total_transporter_rate     = !empty($request->input('total_transporter_rate')) ? $request->input('total_transporter_rate') : 0;
        $total_contractor_rate      = !empty($request->input('total_contractor_rate')) ? $request->input('total_contractor_rate') :0;
        
        $purchase_account_id          = DB::table('default_account')->where('form_type','purchase_return')->where('field_type','sales_account')->first()->account_id ?? 0;
        $sales_tax_input_and_output   = DB::table('default_account')->where('form_type','purchase_return')->where('field_type','sales_tax')->first()->account_id ?? 0;
        $sale_tax_further_tax         = DB::table('default_account')->where('form_type','purchase_return')->where('field_type','further_tax')->first()->account_id ?? 0;
        $sales_man_commission_exp     = DB::table('default_account')->where('form_type','purchase_return')->where('field_type','salesman_commission')->first()->account_id ?? 0;
        $vehicle_exp_account          = DB::table('default_account')->where('form_type','purchase_return')->where('field_type','vehicle_exp')->first()->account_id ?? 0;
        $contractor_exp               = DB::table('default_account')->where('form_type','purchase_return')->where('field_type','contractor_exp')->first()->account_id ?? 0;
        
        $saleman_comm_amount        = !empty($total_salesman_commission) ? $total_salesman_commission : 0;
        $purchase_acc_amount        = $purchase_return->final_total - $total_sale_tax - $total_further_tax;
        $transaction_date           = $input_data['transaction_date'];
        $supp_acc_amount            = $purchase_return->final_total;
        $purchase_acc_amount        = (!empty($purchase_return->add_charges)) ? $purchase_acc_amount - $purchase_return->add_charges : $purchase_acc_amount;
        $purchase_acc_amount        = (!empty($purchase_return->less_charges)) ? $purchase_acc_amount + $purchase_return->less_charges : $purchase_acc_amount;
        
        $old_sell_entgry = AccountTransaction::where("transaction_id",$purchase_return->id);  
        if($old_sell_entgry->count() > 0){
            AccountTransaction::where("transaction_id",$purchase_return->id)->forceDelete();
        }
        //1st entry
        $get_account_id = Account::where("contact_id",$purchase_return->contact_id)->firstOrFail();
        // $pur_no = "purchase-return-".rand(100,1000);
        $pur_no = "purchase-return-".$purchase_return->id;
        
        
        $ob_transaction_data = [
            'amount' => $supp_acc_amount,
            'account_id' => $get_account_id->id, // Supplier Account
            'type' => 'debit',
            'reff_no' => $pur_no,
            'note' => "Supplier Account",
        ...
        if(!empty($total_sale_tax) && isset($total_sale_tax)){
            $ob_transaction_data = [
                'amount' => $total_sale_tax,
                'account_id' => $sales_tax_input_and_output, //Sales tax input & output
                'type' => 'credit',
                'reff_no' => $pur_no,
                'note' => "Sales tax",
        ...
        if(!empty($total_further_tax) && isset($total_further_tax)){
            $ob_transaction_data = [
                'amount' => $total_further_tax,
                'account_id' => $sale_tax_further_tax, //Sales tax further tax
                'type' => 'credit',
                'reff_no' => $pur_no,
                'note' => "further tax",
        ...
        if (isset($purchase_return->add_charges) && !empty($purchase_return->add_charges)) {
            $ob_transaction_data = [
                'amount' => $purchase_return->add_charges,
                'account_id' => $purchase_return->add_charges_acc_id, //add_charges_account
                'type' => 'credit',
                'reff_no' => $pur_no,
                'note' => "Add Charges",
                'against_id' => $get_account_id->id,
        ...
        if (isset($purchase_return->less_charges) && !empty($purchase_return->less_charges)) {
            $ob_transaction_data = [
                'amount' => $purchase_return->less_charges,
                'account_id' => $purchase_return->less_charges_acc_id, //less_charges_account
                'type' => 'debit',
                'reff_no' => $pur_no,
                'note' => "Less Charges",
                'against_id' => $get_account_id->id,
        ...
        if(isset($purchase_return->sales_man) && !empty($purchase_return->sales_man) && !empty($saleman_comm_amount)){
             //4st entry
            $get_account_id_saleMan = Account::where("contact_id",$purchase_return->sales_man)->firstOrFail();
            $ob_transaction_data = [
                'amount' => $saleman_comm_amount,
                'account_id' => $get_account_id_saleMan->id,
                'type' => 'debit',
                'reff_no' => $pur_no,
                'note' => "COMMISSION EXP",
                'against_id' => $sales_man_commission_exp,
                'sub_type' => 'sell_voucher',
                'po_ref_no' => "N/A",
                'operation_date' => $transaction_date,
                'created_by' => auth()->user()->id,
        ...
        if(isset($purchase_return->transporter_name) && !empty($purchase_return->transporter_name) && !empty($total_transporter_rate)){
            //5st entry
            $get_account_id_transporter = Account::where("contact_id",$purchase_return->transporter_name)->firstOrFail(); 
            $ob_transaction_data = [
                'amount' => $total_transporter_rate,
                'account_id' => $get_account_id_transporter->id,
                'type' => 'debit',
                'reff_no' => $pur_no,
                'note' => "vehicle Exp",
                'against_id' => $vehicle_exp_account,
        ...
        if(isset($purchase_return->contractor) && !empty($purchase_return->contractor) && !empty($total_contractor_rate)){
            //6st entry
            $get_account_id_contractor = Account::where("contact_id",$purchase_return->contractor)->firstOrFail();      
            $ob_transaction_data = [
                'amount' => $total_contractor_rate,
                'account_id' => $get_account_id_contractor->id,
                'type' => 'debit',
                'reff_no' => $pur_no,
                'note' => "Contractor EXP",
                'against_id' => $contractor_exp,
