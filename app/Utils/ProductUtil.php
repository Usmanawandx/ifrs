...
class ProductUtil extends Util
{
    ...
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $product_locations, $combo_variations = [])
    {
        ...
        $product_variation = new VariationLocationDetails;
        $product_variation->product_id = $variation->product_id;
        $product_variation->product_variation_id = $variation->product_variation_id;
        $product_variation->variation_id = $variation->id;
        $product_variation->location_id = $product_locations[0];
        $product_variation->qty_available = "0";
     ...
     * @return boolean
     */
    public function updateProductQuantity($location_id, $product_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true, $is_edit = false)
    {
        ...
        $variation = Variation::where('id', $product->variations[0]->id)
            ->where('product_id', $product_id)
            ->first();
        if ($is_edit) {
            $variation_location_details = VariationLocationDetails::where('variation_id', $product->variations[0]->id)
                ->where('product_id', $product->id)
                ->where('location_id', "12")->firstOrFail(); // ->decrement('qty_available', $old_quantity);
            if ($variation_location_details->qty_available > 0) {
                $variation_location_details->qty_available -= str_replace(',', '', $old_quantity);
        ...
        $variation_location_d = VariationLocationDetails
            ::where('variation_id', $product->variations[0]->id)
            ->where('product_id', $product_id)
            ->where('product_variation_id', $variation->product_variation_id)
            ->where('location_id', $location_id)
            ->first();
        //  dd($variation_location_d);
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        if ($product->enable_stock == 1) {
            ...
            $details = VariationLocationDetails::where('variation_id', $product->variations[0]->id)
                ->where('product_id', $product_id)
                ->where('location_id', $location_id)
                ->first();
    ...
    public function createOrUpdatePurchaseLines(
        ...
        DB::table('valuation_history')->where('ref_no', $transaction->ref_no)->delete();
        foreach ($input_data as $data) {
            ...
            if ($transaction->type == "Purchase_invoice") {
                ...
                $this->updateProductStock("received", $transaction, $data['product_id'], '0', $new_quantity, $data['quantity'], $currency_details, false);

                $variation_location_detail = VariationLocationDetails::where('product_id', $data['product_id'])->first();

                $last_purchase_price = Purchaseline::where('product_id', $data['product_id'])
            ...
            $purchase_line->purchase_price_inc_tax = ($this->num_uf($data['purchase_price'], $currency_details) * $exchange_rate) / $multiplier;
            ...
            $purchase_line->store = !empty($data['store']) ? $data['store'] : null;
            $purchase_line->item_code = !empty($data['item_code']) ? $data['item_code'] : null;
            $purchase_line->item_description = !empty($data['item_description']) ? $data['item_description'] : null;
            ...
            $purchase_line->brand_id = !empty($data['brand_id']) ? $data['brand_id'] : null;
            $purchase_line->purchase_order_line_id = !empty($data['purchase_order_line_id']) ? $data['purchase_order_line_id'] : null;

            $purchase_line->further_tax = !empty($data['further_taax_id']) ? $data['further_taax_id'] : null;
            $purchase_line->salesman_commission = !empty($data['salesman_commission_rate']) ? $data['salesman_commission_rate'] : null;
            $updated_purchase_lines[] = $purchase_line;

            $old_qunty = $data['old_quantity'] ?? '0';

            $quantity_purchased = $data['po_qty_change'] ?? '0';

            $total_quantity = $quantity_purchased + $new_quantity;


            $unit_price_up = !empty($data['unit_price_up']) ? $data['unit_price_up'] : 0;
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $variation = Variation::where('product_id', $data['product_id'])->first();
                $variation->sell_price_inc_tax = round($unit_price_up, 2);
                $variation->save();


                $this->updateProductStock($before_status, $transaction, $data['product_id'], $data['variation_id'], $new_quantity, $old_qunty, $currency_details, true);
            } else {
            ...
            // For check stock hit on GRN or Invoice
            $business_id = session()->get('user.business_id');
            $business = Business::findOrfail($business_id);
            $stock_hit_type = $business->stock_hit_on; //e.g Purchase_invoice, purchase

            // hit stock_history
            if ($transaction->type == $stock_hit_type || $transaction->type == "production_purchase" || $transaction->type == "stock_adjustment") {
                // ingredients for production
                if($transaction->type == "stock_adjustment" &&  $this->num_uf($data['quantity']) < 0){
                    $this->stock_history($data, $transaction, 2, $ingredients);
                }else{
                    $this->stock_history($data, $transaction, 1, $ingredients);
                }
            }
            if($transaction->type == "Purchase_invoice" || $transaction->type == "production_purchase"){
                $this->valuation_history($data, $transaction);
            }
        }
        ...
        if($transaction->type == "Purchase_invoice"){
            $this->purchase_invoice_ledger_entries($transaction);
        }

        PurchaseLine::where('transaction_id', $transaction->id)->delete();
        
        $result = $transaction->purchase_lines()->saveMany($updated_purchase_lines);
     ...
     *
     */
    public function updateProductStock($status_before, $transaction, $product_id, $variation, $new_quantity, $old_quantity, $currency_details, $is_edit = false)
    {
        ...
        if ($status_before == 'received' && $transaction->status == 'received') {
            ...
            $this->updateProductQuantity($transaction->location_id, $product_id, $new_quantity_f, $old_qty, $currency_details, true, $is_edit);
            ...
            //     $old_quantity
            // );
        } elseif ($status_before == 'ordered' && $transaction->status == 'received') {
            // dd("sa");
            $this->updateProductQuantity($transaction->location_id, $product_id, $new_quantity_f, $old_qty, $currency_details, true, $is_edit);
        }
    }
    ...
    //     //     $query->where('p.start_date', $filters['start_date']);
    //     // }
    //     // if (!empty($filters['end_date'])) {
    //     //     $query->where('p.end_date', $filters['end_date']);
    //     // }
    ...
    //     // for greater than and less than stock
    ...
    //         $query->where('p.product_type', $filters['product_type_id']); 
    //     }
