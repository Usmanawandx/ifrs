...
class ProductUtil extends Util
{
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            // if($transaction->type == "Purchase Requisition"){
            // $purchase_line->r_quantity = $new_quantity;
            // }
            // dd();
...
//  dd("sass");
                ...
                // $purchase_line->product_id = $data['product_id'];
                 ...
                 if($transaction->type=="purchase" || $transaction->type=="production_purchase")
                {
                    ...
                    if($variation->sell_price_inc_tax==0.0000)
                    {

           

                    $variation->sell_price_inc_tax +=  str_replace(',', '',$unit_price_up);
                    $variation->save();
                ...
                //   dd($total_quantity);
                    ...
                    $variation->sell_price_inc_tax = $variation->sell_price_inc_tax/$total_quantity;
                    $variation->save();
                    ...
                    $this->updateProductStock("received", $transaction, $data['product_id'],$data['variation_id'],$new_quantity, $purchase_line->quantity, $currency_details,false);
                }
        ...
        // {
            ...
            foreach($updated_purchase_lines as $key => $value)
            {
// dd("sa");
   ...
   if($transaction->type == "purchase_order" && $transaction->purchase_order_ids != null){
    ...
    // dd($updated_purchase_lines);
       ...
       foreach ($updated_purchase_lines as $value) {



        $old=PurchaseLine::where('transaction_id',$transaction->purchase_order_ids)->where('product_id',$value->product_id)->first();
        if($old !="")
         {
        ...
        // dd($old[0]);
          ...
          ->where('transaction_id', $transaction->purchase_order_ids)
          ->where('product_id', $value->product_id)
          ->update(['r_quantity' => $value->quantity+$old->r_quantity]);
         }
