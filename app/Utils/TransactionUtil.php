...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'tandc_title' => $input['tandc_title'] ?? '',
            'tandc_type'  => $input['tandc_type'] ?? '',
            'business_id' => $business_id,
            'location_id' => $input['location_id'],
            'type' => $sale_type,
            'status' => $input['status'],
            'contractor' => !empty($input['contractor']) ? $input['contractor'] : '',
            'prd_contractor' => !empty($input['prd_contractor']) ? $input['prd_contractor'] : '',
            ...
            'posting_date' => !empty($input['posting_date']) ? $input['posting_date'] : null,
            'efective_date' => !empty($input['efective_date']) ? $input['efective_date'] : null,
            ...
            'invoice_date' => !empty($input['invoice_date']) ? $input['invoice_date'] : null,
            'return_date' => !empty($input['return_date']) ? $input['return_date'] : null,
            'transporter_name' => !empty($input['transporter_name']) ? $input['transporter_name'] : null,
            'transaction_account' => !empty($input['transaction_account']) ? $input['transaction_account'] : null,
            'gstno' => !empty($input['gstno']) ? $input['gstno'] : null,
            'delivery_note_no' => !empty($input['delivery_note_no']) ? $input['delivery_note_no'] : null,
            'sale_invoice_no' => !empty($input['sale_invoice_no']) ? $input['sale_invoice_no'] : null,
            'sale_order_no' => !empty($input['sale_order_no']) ? $input['sale_order_no'] : null,
            'vehicle_no' => !empty($input['vehicle_no']) ? $input['vehicle_no'] : null,
            'ntncnic' => !empty($input['ntncnic']) ? $input['ntncnic'] : null,
            'saleType' => !empty($input['saleType']) ? $input['saleType'] : null,
            'sales_man' => !empty($input['sales_man']) ? $input['sales_man'] : null,
            'pay_type' => !empty($input['pay_type']) ? $input['pay_type'] : null,
            'discount_amount' => $uf_data ? $this->num_uf($input['discount_amount']) : $input['discount_amount'],
            'tax_amount' => $invoice_total['tax'],
            'add_charges' => !empty($input['add_charges']) ? $input['add_charges'] : null,
            'less_charges' => !empty($input['less_charges']) ? $input['less_charges'] : null,
            'add_charges_acc_id' => !empty($input['add_charges_acc_id']) ? $input['add_charges_acc_id'] : null,
            'less_charges_acc_id' => !empty($input['less_charges_acc_id']) ? $input['less_charges_acc_id'] : null,
        ...
        if($input['type'] == "sale_invoice"){
            $this->sale_invoice_ledger_entries($input, $transaction, $final_total, $user_id);
        }else if($input['type'] == "sale_return_invoice"){
            $this->sale_return_ledger_entries($input, $transaction, $final_total, $user_id);
        }
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $update_date = [
            ...
            'sale_invoice_no' => !empty($input['sale_invoice_no']) ? $input['sale_invoice_no'] : null,
            'sales_man' => !empty($input['sales_man']) ? $input['sales_man'] : null,
            'ref_no' => !empty($input['ref_no']) ? $input['ref_no'] : null,
             'pay_type' => !empty($input['pay_type']) ? $input['pay_type'] : null,
            // 'delivery_note_no' => !empty($input['delivery_note_no']) ? $input['delivery_note_no'] : null,
            'return_date' => !empty($input['return_date']) ? $input['return_date'] : null,
            'staff_note' => !empty($input['staff_note']) ? $input['staff_note'] : null,
        ...
        if($transaction->type == "sale_invoice"){
            $this->sale_invoice_ledger_entries($input, $transaction, $final_total, $user_id);
        }else if($transaction->type == "sale_return_invoice"){
            $this->sale_return_ledger_entries($input, $transaction, $final_total, $user_id);
        }
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        DB::table('stock_history')->where('ref_no', $transaction->ref_no)->delete();
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $product_details = Product::where("id", $product["product_id"])
                    ->with('variations', 'product_locations')->first();

                  if ($transaction->type == "sale_return_invoice") {

                    VariationLocationDetails::where('variation_id', $product_details->variations[0]->id)
                        ->where('product_id', $product_details->id)
                        ->where('location_id', "12")
                        ->decrement('qty_available', $product['old_quantity']);

                    VariationLocationDetails::where('variation_id', $product_details->variations[0]->id)
                        ->where('product_id', $product_details->id)
                        ->where('location_id', "12")
                        ->increment('qty_available', str_replace(',', '', $product['quantity']));
                } elseif ($transaction->type == "delivery_note") {

                    VariationLocationDetails::where('variation_id', $product_details->variations[0]->id)
                        ->where('product_id', $product_details->id)
                        ->where('location_id', "12")
                        ->increment('qty_available', $product['old_quantity']);

                    VariationLocationDetails::where('variation_id', $product_details->variations[0]->id)
                        ->where('product_id', $product_details->id)
                        ->where('location_id', "12")
                        ->decrement('qty_available', str_replace(',', '', $product['quantity']));
                }
                ...
                $uf_unit_price_inc_tax = $uf_data ? $this->num_uf($product['unit_price']) : $product['unit_pricef'];
                $line = [
                    'product_id' => $product['product_id'],
                    'variation_id' => $product['variation_id'],
                    'quantity' =>  $uf_quantity * $multiplier,
                    'ing_quantity' =>  !empty($product['ing_quantity']) ? $product['ing_quantity'] : 0,
                    'unit_price_before_discount' => $unit_price_before_discount,
                    'unit_price' => $unit_price,
                    'line_discount_type' => !empty($product['line_discount_type']) ? $product['line_discount_type'] : 0,
                    ...
                    'further_tax' => (isset($product['further_taax_id'])) ? $product['further_taax_id'] : '',
                    'salesman_commission' => (isset($product['salesman_commission_rate'])) ? $product['salesman_commission_rate'] : '',
                    ...
                    'sub_unit_id' => !empty($product['sub_unit_id']) ? $product['sub_unit_id'] : 0,
                    'discount_id' => !empty($product['discount_id']) ? $product['discount_id'] : 0,
                    'res_service_staff_id' => !empty($product['res_service_staff_id']) ? $product['res_service_staff_id'] : 0,
                    'res_line_order_status' => !empty($product['res_service_staff_id']) ? 'received' : 0,
                    'so_line_id' => !empty($product['so_line_id']) ? $product['so_line_id'] : 0,
            ...
            // hit stock_history
            if($transaction->type == 'delivery_note'){
                $this->stock_history($product, $transaction, 2);
            }elseif($transaction->type == 'sale_return_invoice'){
                $this->stock_history($product, $transaction, 1);
            }
        }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $product_variation = !empty($line->variations->product_variation) ? $line->variations->product_variation : '' ;
    ...
    */
    public function getListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                    ...
                    ->leftJoin(
                        'transactions AS Pr_no',
                        'transactions.purchase_order_ids',
                        '=',
                        'Pr_no.id'
                    )
                    ->leftJoin(
                        'transactions AS pr_nn',
                        'transactions.purchase_order_ids',
                        '=',
                        'pr_nn.id'
                    )
                    ...
                    ->select(
                        ...
                        DB::raw('DATE_FORMAT(transactions.posting_date, "%d-%m-%Y") as posting_date'), // Format posting_date
                        'transactions.ref_no',
                        'contacts.name',
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->select(
                    ...
                    'contacts.contact_id',
                    'contacts.supplier_business_name',
                        'contacts.ntn_cnic_no as ntn_cnic_no',
