...
const Categories = ({ navigation }) => {
  ...
  .catch((error)=>{
    console.log("something wrong in category")
  })
...
return(
  <View style={styles.container}>
    {/* {category.slice(0,10).map((item) => (
      <TouchableOpacity
        key={item.id}
        onPress={() => navigation.navigate('Category', { title: item.name,isfor:'category',id:item.id })}
      >
        <View style={styles.button}>
          <Image
            source={{ uri:`https://ecpmarket.mywheels.pk/assets/images/categories/${item.app_photo}`}}
            resizeMode="contain"
            style={styles.image}
          />
