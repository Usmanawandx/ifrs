<div class="table-responsive">
    <table class="table table-striped ajax_view hide-footer dataTable table-styling table-hover table-primary"
        id="report_table">
        <thead>
            <tr>
                <th>@lang('messages.date')</th>
                <th>@lang('sale.customer_name')</th>
                <th>@lang('Transaction No')</th>
                <th></th>
                <th>NTN / CNIC</th>
                <th>Vehicle</th>
                <th>@lang('Remarks')</th>
                <th>Weight</th>
                <th>Amount</th>
                <th>Tax</th>
                <th>Net Amount</th>
            </tr>
        </thead>
        <tbody>
            @php
                $grandTotal = 0;
                $grandAddCharges = 0;
                $grandLessCharges = 0;
                $grandAddLessCharges = 0;
                $grandQty = 0;
                $grandWeight = 0;
                $grandSaleTax = 0;
                $grandFurtherTax = 0;
            @endphp
            @foreach ($data as $item)
                @php
                    $Totalamount = 0;
                    $add_less_charges = (!empty($item->less_charges) ? -$item->less_charges : $item->add_charges) ?? 0;
                    $TotalQty = 0;
                    $TotalWeight = 0;
                    $TotalSalesTax = 0;
                    $TotalFurtherTax = 0;
                    $grandTotal += $item->final_total;
                    $grandAddCharges += $item->add_charges;
                    $grandLessCharges += $item->less_charges;
                    $grandAddLessCharges += (!empty($item->less_charges) ? '-' . $item->less_charges : $item->add_charges) ?? '0.00';
                @endphp

                <tr>
                    
                    <td>{{ @format_date($item->transaction_date) }}</td>
                    <td>{{ $item->supplier_business_name }}</td>
                    <td>{{ $item->ref_no }}</td>
                    <td>{{ $item->pro_no }}</td>
                    <td>{{ $item->ntn_cnic_no }}</td>
                    <td>{{ !empty($item->vehicle) ? $item->vehicle : $item->vehicle_no }}</td>
                    <td>{{ $item->additional_notes }}</td>
                    @foreach ($item->purchase_lines as $item)
                        @php
                            $amount = $item->quantity_returned * $item->pp_without_discount;
                            $saletaxRate = $item->line_tax->amount ?? 0;
                            $salestax = $saletaxRate > 0 ? ($amount * $saletaxRate) / 100 : 0;
                            $furthertaxRate = $item->further_taxs->amount ?? 0;
                            $furtherTax = $furthertaxRate > 0 ? ($amount * $furthertaxRate) / 100 : 0;

                            $Totalamount += $amount;
                            $TotalQty += $item->quantity_returned ?? 0;
                            $TotalWeight += $item->product->product_custom_field1 * $item->quantity_returned ?? 0;
                            $TotalSalesTax += $salestax;
                            $TotalFurtherTax += $furtherTax;

                            $grandQty += $item->quantity_returned ?? 0;
                            $grandSaleTax += $salestax;
                            $grandFurtherTax += $furtherTax;
                            $grandWeight += $item->product->product_custom_field1 * $item->quantity_returned ?? 0;
                        @endphp
                    @endforeach
                    <td class="text-right">{{ number_format($TotalWeight, 2) }}</td>
                    <td class="text-right">{{ number_format($Totalamount, 2) }}</td>
                    <td class="text-right">{{ number_format(($TotalSalesTax + $grandFurtherTax), 2) }}</td>
                    <td class="text-right">{{ number_format(($Totalamount + $TotalSalesTax + $grandFurtherTax), 2) }}</td>
                </tr>
                
            @endforeach
            <tr>
                <td><b>Add/Less Charges</b></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td class="text-right"><b>{{ number_format($grandAddLessCharges, 2) }}</b></td>
            </tr>
            <tr>
                <td><b>Grand Total</b></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td><b style="float: left">Qty :</b><b style="float: right">{{ number_format($grandQty, 2) }}</b></td>
                <td><b style="float: left">Weight :</b><b style="float: right">{{ number_format($grandWeight, 2) }}</b>
                </td>
                <td>
                    <b style="float: left">Sale Tax :</b>
                    <b style="float: right">{{ number_format($grandSaleTax, 2) }}</b>
                </td>
                <td>
                    <b style="float: left">Further Tax :</b>
                    <b style="float: right">{{ number_format($grandFurtherTax, 2) }}</b>
                </td>
                <td>
                    <b style="float: left">Amount :</b>
                    <b style="float: right">{{ number_format($grandTotal, 2) }}</b>
                </td>
            </tr>
        </tbody>
    </table>

</div>
