    ...
    <table class="table table-striped ajax_view hide-footer dataTable table-styling table-hover table-primary"
        ...
        <tbody>
            ...
            @foreach ($data as $item)
                ...
                @endphp
                <tr  style="font-size: 12px" class="customer_name">
            ...
            @foreach ($item->purchase_lines as $item)
                ...
                <tr class="product-row nested-tbody" style="border: none !important;">
                    ...
                    <td class="net_weight text-right">
                        {{ number_format($item->product->product_custom_field1 * $item->quantity_returned ?? 0, 2) }}
                    </td>
                    <td class="text-right">{{ number_format($item->pp_without_discount) }}</td>
                    <td class="text-right">{{ number_format($salestax, 2) }}</td>
                    <td class="further_tax text-right">{{ number_format($furtherTax, 2) }}</td>
