                ...
                @if ($item->purchase_lines)
                    ...
                    @foreach ($item->purchase_lines as $purchase_line)
                        @if ($purchase_line->purchasetransaction)
                            @php
                                $i++;
                                if (isset($purchase_line, $purchase_line->purchasetransaction, $purchase_line->purchasetransaction->purchase_order_ids)) {
                                    $grn_no = DB::table('transactions')
                                        ->where('id', $purchase_line->purchasetransaction->purchase_order_ids)
                                        ->first()->ref_no;
                                }
                                ...
                                $grandQty += $purchase_line->quantity;
                                $grandWeight += $item->product_custom_field1 * $purchase_line->quantity;
                            ...
                            <tr>
                                ...
                                <td>{{ @format_date($purchase_line->purchasetransaction->transaction_date) }}</td>
                                <td>{{ $purchase_line->transaction->contact->supplier_business_name }}</td>
                                <td>{{ $purchase_line->purchasetransaction->ref_no }}</td>
                                <td>{{ $grn_no ?? '' }}</td>
                                <td>{{ $purchase_line->brand->name ?? '' }}</td>
                                <td>{{ $purchase_line->sell_line_note }}</td>
                                <td class="text-right">{{ number_format($purchase_line->quantity, 2) }}</td>
                                <td class="text-right">{{ number_format($item->product_custom_field1 * $purchase_line->quantity,2) }}</td>
