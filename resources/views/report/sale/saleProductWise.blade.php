...
@section('content')
    ...
    <section class="content-header no-print">
        <h1>Sale Product Wise Report </h1>
    </section>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function loadSaleReport(summary = null) {
                $.ajax({
                    data: {
                        start_date: $('#expense_date_range').data('daterangepicker').startDate.format('YYYY-MM-DD'),
                        end_date: $('#expense_date_range').data('daterangepicker').endDate.format('YYYY-MM-DD'),
            ...
            $(document).on('change',
                '#sell_list_filter_products,#products,#product_type,#product_category,#product_sub_category,#sell_list_filter_location_id, #sell_list_filter_customer_id, #sell_list_filter_payment_status, #created_by, #sales_cmsn_agnt, #service_staffs, #shipping_status, #sell_list_filter_source, #sell_list_filter_sale_type, #sell_list_filter_transporter, #sell_list_filter_vehicle, #sell_list_filter_salesman, #sell_list_filter_accounts, #sell_list_filter_accountsType, #expense_date_range, .vehicles_input',
                function() {
                    $('#summary_report, #hide_weight, #hide_further_tax').trigger('ifChanged');
                });


            $('#summary_report, #hide_weight, #hide_further_tax, #show_zero, #products, #product_type, #product_category, #product_sub_category')
                .off('ifChanged').on('ifChanged', function(e) {
