  ...
  <div class="modal-body">
    ...
    <div class="row invoice-info">
      ...
      <div class="col-sm-4 invoice-col">
        ...
        <b>@lang('Expected Date'):</b> {{ @format_date($purchase->expected_date) }}<br />
        @if(!empty($purchase->status))
        <b>@lang('purchase.purchase_status'):</b> @if($purchase->type == 'purchase_order'){{$po_statuses[$purchase->status]['label'] ?? ''}} @else {{ __($purchase->status) }} @endif<br>
        @endif
        ...
        <b>PO #:</b> {{ __($purchase->po_ref_no) }}<br>
        @endif
        @if(!empty($purchase->grn_ref_no))
        <b>GR #:</b> {{ __($purchase->grn_ref_no) }}
        @endif
