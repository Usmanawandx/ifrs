<style>
  ...
  /*  }*/
    ...
    /*}*/
     ...
     .total_am{
         background-color: #1b5596 !important; margin-bottom: 0px; margin-top: -35px; color: #fff;
     }
  ...
  <div style="background-color: #FFF !important; max-width:100% !important;" class="photo">
          ...
          <div class="row">
              <div class="col-xs-6 col-md-6 col-xs-6">
               ...
               <div class="col-xs-6 col-md-6 col-xs-6">
                  ...
                  @endphp
                  @endif
                  <h3 class="modal-title prt_title pull-right" id="modalTitle" style="font-weight:600">{{ $title }}</h3>
              </div>
          </div>
    </div>
    ...
    <div class="modal-body invoice_modal">
        ...
        <div class="row invoice-info invoice-info-p">
          <div class="col-sm-8 col-xs-8">
  
            <h3 style="font-size:16px; font-weight:bolder;">{{ $purchase->business->name }}</h3>
            <p>{{ $purchase->location->name }}</p><br>
            @if(!empty($purchase->location->landmark))
            <p>{{$purchase->location->landmark}}</p><br>
            @endif
            @if(!empty($purchase->location->city) || !empty($purchase->location->state) || !empty($purchase->location->country))
            <p>{{implode(',', array_filter([$purchase->location->city, $purchase->location->state, $purchase->location->country]))}}</p><br>
            @endif
            ...
            <p>@lang('contact.mobile'): {{ !empty($purchase->location->mobile) ? $purchase->location->mobile : Auth::user()->contact_number }}</p><br>
            
            @if(!empty($purchase->location->email))
            <p>@lang('business.email'): {{$purchase->location->email}}</p><br>
            @endif
            ...
            <p> Printer on:</p><br>
              <p><b>Supplier:</b></p><br>
              @if(!empty($purchase->contact->supplier_business_name))
              <h4 style="font-size:14px; font-weight:bolder;" >{{ $purchase->contact->supplier_business_name }}</h4>
              @endif
              <p>GST NO: {{$purchase->contact->gst_no??''}} </p><br>
              <p>NTN/CNIC NO: {{$purchase->contact->ntn_cnic_no??''}}</p><br>
              @elseif($purchase->type == 'purchase')
              <p> Printer on:</p><br>
              <p>Supplier:</p><br>
              @if(!empty($purchase->contact->supplier_business_name))
            ...
            @endif
              ...
              <p>Supplier Address:  {{$purchase->location->city}}</p><br>
              <p>Telephone No: {{$purchase->contact->mobile??''}}</p>
            
            
        </div>
          <div class="col-sm-2 col-xs-2">
              @if($purchase->type == 'purchase_order')
              <p>@lang('purchase.ref_no')</p>
              <br />
              <p>Document Date</p>
              <br />
              <p>Delivery Date</p>
              ...
              <p>@lang('purchase.purchase_status')</p>
              <br>
              @endif
              @if(!empty($purchase->payment_status))
              <p>@lang('purchase.payment_status')</p>
              <br>
              @endif
          ...
          <div class="col-sm-2 col-xs-2">
              ...
              <p> :&nbsp;&nbsp;&nbsp; </p><br />
              <p> :&nbsp;&nbsp;&nbsp; {{ @format_date($purchase->delivery_date) }}</p><br />
              <p> :&nbsp;&nbsp;&nbsp; Item P.O</p><br />
              <p> :&nbsp;&nbsp;&nbsp; {{ @format_date($purchase->posting_date) }}</p><br />
              <p> :&nbsp;&nbsp;&nbsp; </p><br />
              <p> :&nbsp;&nbsp;&nbsp; </p><br />
          ...
          <div class="table-responsive third.row">
            <table class="table bg-gray main">
              ...
              <tr>
                ...
                <td class="text-right"><span  data-is_quantity="true" data-currency_symbol="false">{{ $purchase_line->quantity }}</span> </td>
                ...
                <td class="no-print text-right"><span class="" data-currency_symbol="true">{{$ttl=  round(($purchase_line->quantity /$purchase_line->product->unit->base_unit_multiplier) * $purchase_line->pp_without_discount,2) }}</span></td>
                @else
                <td class="no-print text-right"><span class="" data-currency_symbol="true">{{$ttl=  $purchase_line->quantity * $purchase_line->purchase_price }}</span></td>
                @endif
                
                @if($purchase_line->tax_id =="")
                
                <td class="no-print text-right"><span class="" data-currency_symbol="true">0</span></td>
                @else
                   <td class="no-print text-right"><span class="" data-currency_symbol="true">{{ ($ttl * ($purchase_line->line_tax->amount ?? 0)) /100  }}</span></td>
                @endif
                ...
                <td class="text-right"><span class="" data-currency_symbol="true">{{ $purchase_line->purchase_price_inc_tax }}</span></td>
                @endif
                @if($purchase->type != 'purchase_order' && $purchase->type != 'purchase' && $purchase->type != 'purchase_return' && $purchase->type != 'Purchase_invoice')
                @php
      ...
      <div class="row mian-foot-row">
        ...
        <div class="col-md-8 col-sm-8 col-xs-8 mian-foot-first mian-foot-p">
          ...
          <p>{{ $purchase->additional_notes  }}</p><br>
          <p>Total Amount In Words:<b>{{$final_total??''}}</b></p>
          <br>
        ...
        <div class="col-md-4 col-sm-4 col-xs-4 mian-foot-last">
          <div class="table-responsive">
            <table class="table table_bottom table_bottom_p">
              <!-- <tr class="hide">
