...
<section class="content-header no-print">
    <h1 >Purchase Requisition
        {{-- <span class="pull-right" style="margin-top:4px;">Transaction No: {{$t_no }}</span> --}}
    </h1>
</section>
...
<section class="content no-print">
    ...
    @endcomponent
    @component('components.widget', ['class' => 'box-primary', 'title' => __('Purchase Requisation')])
        @can('purchase.purchase_req.add')
            @slot('tool')
                <div class="box-tools">
                    <a class="btn btn-block btn-primary" href="{{action('PurchaseOrderController@create_requision')}}">
                    <i class="fa fa-plus"></i> @lang('messages.add')</a>
                </div>
                <div class="box-tools" style="margin-right: 10px;">
                    <a class="btn btn-block btn-primary" href="{{ url('/DeleteRecords', ['type' => 'Purchase Requisition']) }}" style="display:none;">
                    <i class="fa fa-trash"></i> @lang('Delete Records')</a>
                </div>
            @endslot
        @endcan

        <table class="table table-bordered table-striped ajax_view hide-footer dataTable table-styling table-hover table-primary" id="purchase_requision_table" style="width: 100%;">
            <thead>
                <tr>
...
$(document).ready( function(){
        //Purchase table
        var i = 1;
        purchase_order_table = $('#purchase_requision_table').DataTable({
            processing: true,
            serverSide: true,
            aaSorting: [[1, 'desc']],
            scrollY: "75vh",
            ...
            scrollCollapse: true,
            ajax: {
                url: '{{action("PurchaseOrderController@index_requision")}}',
                data: function(d) {
        ...
        $(document).on(
            'change',
            '#po_list_filter_location_id, #po_list_filter_supplier_id, #po_list_filter_status, #shipping_status,#product_type,#purchase_type',
            function() {
