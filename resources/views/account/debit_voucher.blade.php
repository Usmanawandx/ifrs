...
@section('content')
    ...
    <section class="content">
        ...
        {!! Form::open([
            'url' => action('AccountController@debit_voucher_create'),
            'method' => 'post',
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input required class="form-control voucher_date" type="date" value="<?php echo date('Y-m-d'); ?>"
                                name="date">
                        </div>
                    </div>
                </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover voucher_id" id="debtAccVoucher">
                    ...
                    <tbody id="debitvoucher">
                        <tr>
                            <td>
                                <button class="btn btn-danger red" type="button" value="Delete" onclick="remove_row(this)"><i
                                        class="fa fa-trash"></i></button>
                            </td>
                            <td><input type="text" name="txtCode[]" value="" class="form-control acc_code"
                                    id="txtCode_1" readonly=""></td>
                            <td>
                                <select class="form-control acc_name select2" name="debit_head[]" required
                                    onchange="get_code(this)">
                                    <option value="">Select</option>
                                    <?php foreach($other_accounts as $key => $value) { ?>
                                    <option value="<?php echo $value->id; ?>" account_no="<?php echo $value->account_number; ?>"><?php echo $value->name; ?>
                                    </option>
                                    <?php } ?>
                                </select>
                            ...
                            <td>
                                <input type="file" name="attachment_credit[]" class="form-control" />
                            </td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="2"><a id="add_more" class="btn btn-info" name="add_more"
                                    onclick="add_row(this)">Add Row</a></td>

                            <td colspan="3" class="text-right"><label for="reason"
        ...
        function get_code(el) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var text = (data.balance > 0) ? data.balance + ' Dr: ' : (data.balance < 0) ? data.balance
                        .replace('-', '') + ' Cr: ' : null;
