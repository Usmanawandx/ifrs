...
@if(explode('-',$cv[0]['reff_no'])[0] == 'CRV')
    @section('title', 'Cash Received Voucer')
@else
   @section('title', __('Recipt Voucher'))
@endif
...
@section('content')
    ...
    <section class="content">
        ...
        {!! Form::open([
            'url' => action('AccountController@cv_update', ['reff_no' => $cv[0]['reff_no']]),
            'method' => 'post',
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input required class="form-control" type="date" value="<?php echo date('Y-m-d', strtotime($cv[0]['operation_date'])); ?>" name="date">
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="hidden" name="attachment_one" value="{{ $debit_acc_id['attachment'] }}" class="form-control" />
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <textarea name="remarks" class="form-control">{{ $cv[0]['note'] }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <table class="table table-bordered table-hover voucher_id" id="debtAccVoucher">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" width="15%">Attachment<i class="text-danger">*</i></th>
                            <th class="text-center" width="11%">Amount<i class="text-danger">*</i></th>
                        </tr>
                    </thead>
                    <tbody id="debitvoucher">
                    
                    @foreach($cv as $index => $val )
                        <tr>
                            <input type="hidden" name="id" class="id" value="{{$cv[0]->id}}" >
                            ...
                            <td class="" width="200p">
                                ...
                                <select class="form-control select2 account_id" name="debit_head[]" onchange="get_code(this)"
                                    required>
                                    <option value="">Select</option>
                                    <?php foreach($other_accounts as $key => $value) {?>
                                    @if($value->id ==$cv[$index]->account_id)
                                    <option value="<?php echo $value->id; ?>" selected data-contact_id="{{$value->contact_id}}" ><?php echo $value->name; ?></option>
                                    @else
                            ...
                            <td><input type="number" name="txtAmount[]" value="{{ $val['amount'] }}"
                                    class="form-control total_price text-right" id="txtAmount_1" min="1" onkeyup="total_amount()"
        ...
        <div class="col-sm-12 fixed-button">
            <div class="text-center">
                 <button type="submit" class="btn-big btn-primary">Update</button>
