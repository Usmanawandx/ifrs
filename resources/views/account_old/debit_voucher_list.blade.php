       ...
       @component('components.widget', ['class' => 'box-primary', 'title' => 'All VOUCHERS'])
        @if (auth()->user()->can('account.payment_vouchers.add'))
           @slot('tool')
                <div class="box-tools">
                    @if(request('type') == 'cash_payment_voucher')
                        <a class="btn btn-block btn-primary" href="{{action('AccountController@debit_voucher',['type' => 'cash_payment_voucher'])}}">
                    @else
                        <a class="btn btn-block btn-primary" href="{{action('AccountController@debit_voucher')}}">
                    @endif
         ...
         <tbody>
            ...
            <tr>
                <td>
                    ...
                    @if (auth()->user()->can('account.payment_vouchers.edit'))
                      <a href="/account/dv_edit/{{$v->reff_no}}" class="btn btn-sm btn-success btn-edt" ><i class="fas fa-edit"></i></a>
                    @endif
                    @if (auth()->user()->can('account.payment_vouchers.delete'))
                      <a href="/account/dv_delete/{{$v->reff_no}}" class="btn btn-sm btn-danger btn-dlt" ><i class="fas fa-trash"></i></a>
                    @endif
                 </td>
