        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="form-group col-sm-12 @if (empty($unit->base_unit_id)) hide @endif" id="base_unit_div">
                    <table class="table">
                        <tr>
                            <th style="vertical-align: middle;"></br>Quantity In <span
                                    id="unit_name">{{ $unit->actual_name }}</span></th>
                            <th style="vertical-align: middle;"></br>=</th>
                            <td style="vertical-align: middle;">
                                </br>
                                {!! Form::text(
                                    'base_unit_multiplier',
                                    !empty($unit->base_unit_multiplier) ? $unit->base_unit_multiplier : null,
                                    ['class' => 'form-control input_number', 'placeholder' => __('lang_v1.times_base_unit')],
                                ) !!}</td>
                            <td style="vertical-align: middle;">
                                <b>PURCHASE UNIT </b>
                                @if ($unit->is_purchase_unit == 1)
                                    <input type="checkbox" name="is_purchase_unit" checked>
                                @else
                                    <input type="checkbox" name="is_purchase_unit">
                                @endif
