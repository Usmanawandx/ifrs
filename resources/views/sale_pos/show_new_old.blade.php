...
<div class="modal-dialog modal-xl" role="document">
   <div class="modal-content invoice_modal" id="modal-content" style="padding: 0px 15px;">
            ...
            <div class="row">
                ...
                <div class="col-xs-8 col-md-8 col-xs-8">
                    @if(!empty(Session::get('business.logo')))
                      <img src="{{ asset( 'uploads/business_logos/' . Session::get('business.logo') ) }}" width="200" height="120" alt="Logo">
                    @endif
                </div>
                 <div class="col-xs-4 col-md-4 col-xs-4">
                   ...
                   @elseif($sell->type == 'sale_invoice') 
                    ...
                    <h1 class="modal-title commercial_invoice prt_title" style="font-weight:600">Commercial Invoice</h1>
                    <h1 class="modal-title sale_tax_invoice prt_title"  style="font-weight:600">Sales Tax </h1>
                    <h1 class="modal-title sale_tax_product prt_title"  style="font-weight:600">Sales Tax</h1>
                   @endif
                </div>
            </div>
      ...
      <div class=" col-sm-8 col-md-8 col-xs-7 first-col">
        ...
        <h3 style="font-size:20px;font-weight:bolder;">{{ Session::get('business.name') }} </h3>
    ...
    <p>STRN : {{$sell->contact->ntn_cnic_no??''}}</p>
    ...
    <br/>
      ...
      @else
      <p>  :&nbsp;&nbsp;&nbsp;&nbsp;{{date("d-m-Y",strtotime($sell->delivery_date))}}</p>
      @endif
      @if($sell->type == 'sales_order' || $sell->type == 'sale_invoice')
      </br>
    ...
    @endif
      ...
      @endif
            ...
            @if($sell->type=="sale_invoice")
                <table class="table " id="only_item_wise">
                    <tbody><tr>
                    <th style="
                    width: 38%;
                "></th>
               ...
               <div class="row">
                  ...
                  <div class="col-md-6 col-sm-12 col-xs-6 @if($sell->type == 'sales_order') col-md-offset-8 col-xs-8 @endif" style="margin-left: 66.7%;">
                     <div class="table-responsive amount_p" style="width: 65.1%;margin-top: -60px;">
                  ...
                  <div class="col-md-4 col-sm-4 col-xs-4 pull-right hide_div_org" style="@if($sell->type == 'delivery_note') display: none; @endif">
                     <div style="background-color: #1b5596; margin-bottom: 0px; margin-top: -35px;" class="total_am">
                        <h3 style="color: #FFF; padding: 5px;" id="heading">
                            @if($sell->type == 'sales_order' || $sell->type == 'delivery_note')
                                <span style="font-size: 20px; font-weight: bold;">Total Quantity</span>
                            @else
                                <span style="font-size: 20px; font-weight: bold;">Total</span> 
                            @endif
                             @if($sell->type == 'sales_order')
                                <p class="pull-right total_qty" style="font-size: 20px; font-weight: bold;">{{ $total_qty  }}</p>
                                <p class="pull-right total_amount" style="font-size: 20px; font-weight: bold;">{{$sell->total_before_tax - $sell->discount_amount  }}</p>
                              @elseif($sell->type == 'delivery_note')
                                <p class="pull-right total_qty" style="font-size: 20px; font-weight: bold;">{{ $total_qty  }}</p>
                                @elseif($sell->type == 'sale_invoice')
                                <p class="pull-right total_amount" style="font-size: 20px; font-weight: bold; margin-top: 0px;"><span class="display_currency" data-currency_symbol="true">{{$sell->total_before_tax - $sell->discount_amount  }}</span></p>
                                <p class="pull-right total_qty" style="font-size: 20px; font-weight: bold; display: none;">{{ $total_qty  }}</p>
                            @endif
                     ...
                     <div id="weight">
                        <h4>
                           Gross Weight:
                           <p class="pull-right" style="font-size: 20px">{{ $sell->gross_weight??'0' }}</p>
                        </h4>
                        
                        <h4>
                           Net Weight:
                           <p class="pull-right" style="font-size: 20px">{{ $sell->net_weight ??'0'}}</p>
                        </h4>
                     </div>
                     {{--@endif --}}
                  </div>
                  ...
                  <div class="col-md-4 col-sm-4 col-xs-4 col-xs-4 pull-right hide_div_dup" style="@if($sell->type != 'delivery_note') display: none; @endif">
                     <div style="background-color: #1b5596; margin-bottom: 0px; margin-top: -90px;" class="total_am">
                        <h3 style="color: #FFF; padding: 5px;" id="heading">
                            @if($sell->type == 'sales_order' || $sell->type == 'delivery_note')
                                Total Quantity
                            @else
                            ...
                            @endif
                             @if($sell->type == 'sales_order')
                                ...
                                <p class="pull-right total_qty" style="font-size: 20px">{{ $total_qty  }}</p>
                               <p class="pull-right total_amount" style="font-size: 20px">{{$sell->total_before_tax - $sell->discount_amount  }}</p>
                              @elseif($sell->type == 'delivery_note')
                                <p class="pull-right total_qty" style="font-size: 20px">{{ $total_qty  }}</p>
                                @elseif($sell->type == 'sale_invoice')
                                <p class="pull-right total_amount" style="font-size: 20px"><span class="display_currency" data-currency_symbol="true">{{$sell->total_before_tax - $sell->discount_amount  }}</span></p>
                                <p class="pull-right total_qty" style="font-size: 20px; display: none;">{{ $total_qty  }}</p>
                  ...
                  @if($sell->type=="sales_order" || $sell->type == 'sale_invoice' || $sell->type == 'sale_return_invoice')
                  <div class="col-md-6 col-sm-12 col-xs-12 col-xs-6 amount_inverse"  style="font-size:16px; margin-top: -80px;">
                     <span id="amount_in_wrd">AMOUNT IN WORDS:</span>
                     <span class="amount_in_word" style="display: block; margin-top: 15px;"><?php echo AmountInWords($sell->total_before_tax - $sell->discount_amount)?></span>
                  </div>
                  @endif 
               </div>
           ...
           <div class="spane_d">
               ...
               <div class="row">
                  <div class="col-sm-12">
                       <h5 style="margin-bottom: 22px; font-weight: 600;">Terms And Condition</h5>
               ...
               <div class="row" style="margin: 27px; margin-bottom: 0px;">
                    <!--<br><br><br>-->
                  <hr>
...
<script type="text/javascript">
    ...
    $(document).on('change', 'input:radio[name="rate_radio"]', function(){
        if ($(this).is(':checked') && $(this).val() == 'with_rate') {
            ...
            $('.item_delivry_qty, .item_unit_total, .item_unit_price, .total_sm_table, .amount_inverse, .total_amount, .hide_div_org').show();
        }else{
            $('.item_description, .item_delivry_qty, #weight, .total_qty, .hide_div_dup').show();
            $('.item_unit_total, .item_unit_price, .total_sm_table, .amount_inverse, .total_amount,#sales_taxx,#add_sales_taxx, .hide_div_org').hide();
        }
    })
