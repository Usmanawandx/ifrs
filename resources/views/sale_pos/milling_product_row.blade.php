@php
	...
	// dd($product);
@endphp
<tr class="product_row" data-row_index="0" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
...
<div class="col-sm-3">
				<div class="form-group">
                    ...
                    <select name="products[0][store]" class="form-control input-sm purchase_line_tax_id" placeholder="'Please Select'" style="width: 90px">
...
<td>
	{{-- {!! $product->sub_sku  !!} --}}
	<input class="form-control product_code" readonly="" id="item_code" name="products[0][item_code]" type="text" value="">
...
<td>
		...
		@else
			{{-- {!! product_name !!} --}}
			<select name="products[{{$row_count}}][product_id]" class="form-control select2 prd_select" id="prd_select"  onchange="get_product_code(this)">
				<option value="">Please Select</option>
				@foreach ($prd as $p)
...
<td>
	...
	@if(!empty($is_direct_sell))
		...
		<td  style="  display: none;"@if(!$edit_discount) hide @endif>
		...
		<td  style="  display: none;"class="text-center {{$hide_tax}}">
