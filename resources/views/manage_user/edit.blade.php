...
<section class="content">
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            @php
            $accountIds = explode(',', $user->account_id);
            @endphp
            <div class="form-group">
                {!! Form::label('transaction_account', 'Transaction Account'. ':*') !!}
                <select name="transaction_account[]" class="form-control select2" id="transaction_account" multiple required>
                    @foreach ($transaction_account as $account)
                        <option value="{{ $account->id }}" {{ in_array($account->id, $accountIds) ? 'selected' : '' }}>
                            {{ $account->name }}
                        </option>
                    @endforeach
                </select>
                <label id="transaction_account_error" class="error" style="display: none"></label>
            </div>
        </div>
...
<script type="text/javascript">
  ...
  $('form#user_edit_form').validate({
            ...
            function validateTransactionAccount() {
                // debugger;
                var transactionAccountValue = $('#transaction_account').val();
                var userId = {{$user->id}};
                var accountIds = @json($accountIds);
                var $transactionAccountError = $('#transaction_account_error');

                $.ajax({
                    type: "POST",
                    url: "/manage_user/checkUserTransactionAccount",
                    async: false,
                    data: {
                        transaction_account: transactionAccountValue,
                        user_id: userId,
                    ...
                    success: function(response) {
                      console.log(response);
                        if (response.success === "true") {
                            $transactionAccountError.hide();
                        } else {
                            $transactionAccountError.show().text(response.msg);
                        }
                    }
                });
            }

            $('#transaction_account').on('change', validateTransactionAccount);
            $('#user_edit_form').on('submit', function(event) {
                validateTransactionAccount();
                if ($('#transaction_account_error').is(":visible")) {
                    event.preventDefault();
                }
                setTimeout(() => {
                    $('#submit_user_button').removeAttr('disabled');
                }, 500);
